...
Viz steps (`viz://`) produce visualizations and export steps (`export://`) ship files to external destinations. They are defined in the `etl/steps/viz` and `etl/steps/export` directories and have a similar structure to regular steps. Flags describe where a step writes: chart and explorer steps write to the grapher DB and are run with `--grapher`; bespoke and export steps write to R2 or GitHub and are run with `--export`; static steps only write local files and need no flag.
