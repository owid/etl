...
!!! tip "Quick summary guide"
    ...
    - Create a PR with a staging server with
        ...
        ```
    - Commit files generated by the ETL dashboard and push them.
...
- **Update steps using the ETL Dashboard**:
    ...
    - Copy the recommended command from the output (i.e. `etl pr ...`).
...
- **Run `etl pr ..` to create a PR**
    ...
    This will create a new git branch in your local repository with empty commit, which will be pushed.
