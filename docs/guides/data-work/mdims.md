...
!!! warning "MDIMs and Explorer steps are a work in progress"
    ...
    For recent information please refer to the Slack channel [:fontawesome-brands-slack: #proj-explorers-mdims-convergence](https://owid.slack.com/archives/C08HWKNDG2J). Use this to report any issues or doubts you may have.
...
Instead of creating a `Dataset` object, these create `Chart` or `Explorer` objects and save them to the database. Both share the same data structure (class `Chart`, see [:fontawesome-brands-github: `etl.viz.chart.model`](https://github.com/owid/etl/blob/master/etl/viz/chart/model/core.py)); `Explorer` is a subclass with minor differences due to legacy reasons.
...
MDIMs are defined by a set of indicators that are presented in different views. Each view is defined by a specific dimension value (e.g. `sex="female"`). Ultimately, an MDIM is presented as a chart with additional selection options to switch from one view to another.

To implement an MDIM, you need to create a new step script in the `etl/steps/viz/chart/` directory along with a YAML config file and add a reference to the DAG (just like with any other data step). The step script should load the data dependencies and the default YAML configuration, and save the MDIM to the database.

For details on how to implement an explorer step, first read this section and then continue to [Nuanaces of creating an Explorer :material-arrow-down:](#nuances-of-creating-an-explorer).
...
!!! info "See the complete list of MDIMs and Explorers"
    - [:fontawesome-brands-github:  MDIMs](https://github.com/owid/etl/blob/master/etl/steps/viz/chart/)
    - [:fontawesome-brands-github: Explorers](https://github.com/owid/etl/blob/master/etl/steps/viz/explorer/)

### A simple MDIM
MDIMs can be crafted in various ways, going from totally manual to fully automated. The most common way is to use a combination of manual and automated steps. The automated steps are usually generated by helper functions from the `etl.viz` module.

Below is a simple MDIM configuration file (modified for demo purposes):

```yaml title="etl/steps/viz/chart/covid/latest/covid.deaths.yml"
grapher_schema: "011"
title:
...
def run() -> None:
    ...
    # Create MDIM object
    mdim = paths.create_chart(config)
    # Save to DB
    mdim.save()
```
...
!!! important "`grapher_schema` is required"
    ...
    **There is no default.** Omitting the field fails the step — schema validation rejects it when `create_chart` builds the config, and `Chart.save()` re-checks. That is deliberate: a fallback would resolve an unpinned config to whatever version the repo vendors *on the day the step runs*, which both tells Grapher the config is already current (so the next breaking schema change skips the migration for those views) and silently resolves to a different version the next time the step runs. Once set, leave the pin alone: it is a record of what the config was authored against, which is exactly what lets Grapher migrate it later.
    ...
    **Pin it in one place only.** A view's own `config` may also carry a `$schema`, and Grapher lets that value *win* over the chart pin — while being far less visible, since it usually sits in a Python dict rather than on line 1 of the YAML. ETL logs a warning when a view shadows the chart pin; don't add new ones. If you need to know what version an MDIM actually declares, the chart pin is the answer unless that warning fires.
    ...
    **The pin appears under three different names**, which is worth knowing before you go looking for it:
    ...
    The generated JSON keeps the authored short form because `ChartSet.read()` loads it back through `Chart.from_dict`; only the upsert payload is camelized and resolved to a full URL. So grepping the generated JSON for `grapherConfigSchema` finds nothing even when the pin is working perfectly.
...
### Automated MDIMs
MDIMs are very suitable for those datasets that have a lot of dimensions in them. For these, one can leverage functions like `etl.viz.expand_config`, which programmatically generates all possible views from an indicator (or multiple ones) in a table.
...
!!! question "What does it mean for a dataset to have dimensions?"
    ...
    If you didn't create a dimensional indicator, you can still create an MDIM manually. Or, if you want, you can also add the dimensional information a posteriori ([:fontawesome-brands-github: example](https://github.com/owid/etl/blob/master/etl/steps/viz/chart/energy/latest/energy_prices.py)).
...
You can also combine manually defined views with generated ones. See the `etl.viz` module for available helper functions or refer to examples from `etl/steps/viz/chart/`.
...
As a reference, you can look at the explorer config of the [:fontawesome-brands-github:  Population and Demography explorer](https://github.com/owid/etl/blob/master/etl/steps/viz/explorer/un/latest/un_wpp.manual.config.yml). Below is a snippet of the `common_views` field. In it, we define different general configurations: (i) one for views with `indicator=age_structure`, (ii) one for views with `indicator=age_structure` and `sex=female`, and (iii) one for views with `indicator=age_structure` and `sex=male`.

```yaml title="etl/steps/viz/explorer/un/latest/un_wpp.yaml"
common_views:
  # Age structure
  - dimensions:
...
```yaml title="etl/steps/viz/explorer/who/latest/monkeypox.yaml"
common_views:
- config:
...
The same helpers work for explorers: call `paths.create_explorer` instead of `paths.create_chart`.

!!! note "There is no schema for explorers"


### Explorer examples:
- [:fontawesome-brands-github:  Population and Demography](https://github.com/owid/etl/blob/master/etl/steps/viz/explorer/un/latest/un_wpp.py): Configuration automatically generated (mostly) from dimensions, using `expand_config` and other tools. This one is a good example for explorers with a large number of views.
- [:fontawesome-brands-github:  COVID-19](https://github.com/owid/etl/blob/master/etl/steps/viz/explorer/covid/latest/covid.config.yml): Manually crafted.
- [:fontawesome-brands-github:  Monkeypox](https://github.com/owid/etl/blob/master/etl/steps/viz/explorer/who/latest/monkeypox.py): Manually crafted.
