...
There is on going work in `etl.viz.beta`. In there you can find interesting methods like `combine_explorers`, which lets you combine two explorer objects into one. Since this works also with MDIMs, it can be extremely useful to combine MDIMs automatically-generated from different tables.
