...
schemas/grapher-schema.010.json              ← vendored pin (the only upstream copy in ETL)
    ├─→ etl/viz/chart/model/schema_types.py    generated from it    ← unit test (offline)
...
!!! warning "Never edit `schema_types.py` by hand"
    ...
    `etl/viz/chart/model/schema_types.py` is fully generated by `scripts/generate_schema_types.py`; a unit test fails if it doesn't round-trip. Hand-written types belong in `etl/viz/chart/model/params.py`.
