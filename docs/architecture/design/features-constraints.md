...
We secondly keep record all data dependencies in a directed graph or DAG (see YAML files from [:fontawesome-brands-github: `dag/`](https://github.com/owid/etl/blob/master/dag/)), and forbid steps from using any data as input that isn't explicitly declared as a dependency. This means that the result of any step is a pure function of its inputs.
