"""Tests for etl.viz.chart.core.utils module.

This module tests utility functions for processing charts and views,
including the main chart creation function and view processing logic.
"""

from unittest.mock import Mock, patch

from etl.viz.chart.core.utils import create_chart_from_config, process_views
from etl.viz.chart.model.core import Chart
from etl.viz.chart.model.view import CommonView, View
...
def create_test_config():
    ...
    return {
        # Required for mdims/charts (see `Chart.validate_grapher_schema_pinned`).
        "grapher_schema": "011",
...
class TestProcessViews:
    ...
    def test_process_views_basic(self):
        ...
        # Create mock chart with views
        chart = Mock(spec=Chart)
        view1 = Mock(spec=View)
        view2 = Mock(spec=View)
        chart.views = [view1, view2]
        chart.definitions = None

        dependencies = {"test#indicator1", "test#indicator2"}

        with patch("etl.viz.chart.core.utils.get_tables_by_name_mapping") as mock_mapping:
            mock_mapping.return_value = {"table": ["test/latest/data#table"]}

            process_views(chart, dependencies)
    ...
    def test_process_views_with_common_views(self):
        ...
        # Create mock chart with common views
        chart = Mock(spec=Chart)
        view1 = Mock(spec=View)
        chart.views = [view1]
    ...
    def test_process_views_with_metadata_needed(self):
        ...
        with patch("etl.viz.chart.core.utils.get_tables_by_name_mapping") as mock_mapping:
            ...
            process_views(chart, dependencies, combine_metadata_when_mult=True)
...
class TestCreateChartFromConfig:
    """Test suite for create_chart_from_config function."""

    def test_create_chart_basic(self):
        """Test basic chart creation from config."""
        config = create_test_config()
        dependencies = {"test#indicator1", "test#indicator2"}
        catalog_path = "test/latest/data#table"

        with patch("etl.viz.chart.core.utils.process_views") as mock_process:
            chart = create_chart_from_config(config, dependencies, catalog_path, explorer=False)

            # Verify it returns a Chart
            assert isinstance(chart, Chart)
            assert chart.catalog_path == catalog_path
            assert chart.title == {"title": "Test Chart", "title_variant": "Test Chart Variant"}

            # Verify process_views was called
            mock_process.assert_called_once_with(chart, dependencies=dependencies)
    ...
    def test_create_explorer_from_config(self):
        ...
        with patch("etl.viz.chart.core.utils.process_views") as mock_process:
            with patch.object(Explorer, "validate_schema"):
                explorer = create_chart_from_config(config, dependencies, catalog_path, explorer=True)

                # Verify it returns an Explorer
                assert isinstance(explorer, Explorer)
                assert explorer.catalog_path == catalog_path
                assert explorer.title == {"title": "Test Chart", "title_variant": "Test Chart Variant"}

                # Verify process_views was called
                mock_process.assert_called_once_with(explorer, dependencies=dependencies)

    def test_create_chart_with_validation_disabled(self):
        """Test chart creation with schema validation disabled."""
        config = create_test_config()
        dependencies = {"test#indicator1", "test#indicator2"}
        catalog_path = "test/latest/data#table"

        with patch("etl.viz.chart.core.utils.process_views"):
            chart = create_chart_from_config(config, dependencies, catalog_path, validate_schema=False)

            # Should still create chart successfully
            assert isinstance(chart, Chart)
            assert chart.catalog_path == catalog_path

    def test_create_chart_validation_steps(self):
        ...
        with patch("etl.viz.chart.core.utils.process_views"):
            with patch.object(Chart, "validate_schema") as mock_validate_schema:
                with patch.object(Chart, "validate_views_with_dimensions") as mock_validate_views:
                    with patch.object(Chart, "check_duplicate_views") as mock_check_duplicates:
                        create_chart_from_config(config, dependencies, catalog_path, validate_schema=True)
    ...
    def test_create_chart_config_merging(self):
        ...
        with patch("etl.viz.chart.core.utils.process_views"):
            with patch.object(Chart, "from_dict") as mock_from_dict:
                mock_from_dict.return_value = Mock(spec=Chart)

                create_chart_from_config(config, dependencies, catalog_path, validate_schema=False)
    ...
    def test_create_explorer_config_merging(self):
        ...
        with patch("etl.viz.chart.core.utils.process_views"):
            with patch.object(Explorer, "from_dict") as mock_from_dict:
                ...
                create_chart_from_config(config, dependencies, catalog_path, explorer=True, validate_schema=False)
    ...
    def test_overload_return_types(self):
        ...
        chart_config = create_test_config()
        explorer_config = create_test_explorer_config()
        dependencies = set()
        catalog_path = "test/path#table"

        with patch("etl.viz.chart.core.utils.process_views"):
            # Test Chart return type
            chart = create_chart_from_config(
                chart_config, dependencies, catalog_path, explorer=False, validate_schema=False
            )
            assert isinstance(chart, Chart)

            # Test Explorer return type
            explorer = create_chart_from_config(
...
class TestIntegration:
    ...
    def test_full_pipeline_chart(self):
        """Test complete chart creation and processing pipeline."""
        ...
        with patch("etl.viz.chart.core.utils.process_views") as mock_process:
            with patch.object(Chart, "validate_schema"):
                chart = create_chart_from_config(config, dependencies, catalog_path, validate_schema=True)

                # Verify chart was created with proper structure
                assert isinstance(chart, Chart)
