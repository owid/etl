...
Two limits worth knowing. **The Part 2 redirect does not carry this**: `TARGET_QUERY` in `redirect_to_scatter.py` is one constant for every row, so a reader arriving by a retired slug still gets the linear scatter while a hand-updated article link gets log. `chart_slug_redirects.target_query_param` is per-row, so closing that gap is possible but has not been done. And a reference that forces a **non-scatter** tab still receives the proposal, since the source's log was a global setting — but it is being applied to a view the author's choice was not about, so treat those rows as a judgment call.
