...
- **Its params** — the query string the reference already carries. Two separate things depend on it, and they are easy to conflate. The **replacement URL** merges it over the proposed params with **the reference's values winning**, which is an editorial choice: an article that pinned a country or a year meant to, and a paste should not silently discard that. The **redirect**, by contrast, does not merge at all — any params on the reference erase `target_query_param` completely, so the cell grades 🔴 (params that contradict the retirement's keys) versus ⚠️ (params that merely erase them) and never reports a non-empty query string as fine. A 🔴 row needs a decision rather than a paste.
