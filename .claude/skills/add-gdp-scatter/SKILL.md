...
- **Its params** — the query string the reference already carries, because the replacement URL merges it over the proposed params and **the reference's values win**. A ⚠️ marks the keys that override the retirement's own, which is the row that needs a decision rather than a paste: a reference carrying `tab=chart` lands the reader on a different tab than the retirement intends, and the merge is silent about it.
...
**The Part 2 redirect carries it too**, so the two paths agree: a reader arriving by a retired slug gets the same log scatter as someone following a hand-updated article link. `chart_slug_redirects.target_query_param` is per-row, so `redirect_to_scatter.py` appends `yScale=log` to that row's stored query — the one part of `TARGET_QUERY` that varies by row. Everything Part 2 prints about that row quotes the same per-row query rather than the constant: the ⚠️ collision check (so a reference's own `yScale` is reported as overriding on a log row and left alone on every other), and the narrative-chart replacement instruction (so a replacement built for a log source keeps the log axis).
...
The one thing to check is those params: they arrive as *incoming* params on the redirect, so any of them the stored query also sets wins over it — a narrative chart with its own `tab` or `time`, or on a **log row** its own `yScale`, which lands the reader on a linear view where the retirement carries `log`. The checked keys are read from that row's stored query rather than a fixed list, so the `yScale` case is reported on log rows and correctly ignored everywhere else. The script lists every narrative chart on the sources with its params and says which way each will land.
...
1. **Create** the replacement from `/grapher/<target-slug>?tab=scatter&time=latest&country=` — plus `&yScale=log` if the parent is a log row, which is why the script prints that row's own stored query rather than the constant — using that chart's own **"Create narrative chart"** control, under a new kebab-case name. Use the control, not a bare create link — it parents to the view on screen. Entity selection and other controls open at the target's defaults and authored FAUST never transfers, so redo those by hand.

   **That URL is the scatter view, NOT the old chart's params merged over it.** The handoff builds this column with `create_from_url`, not `replacement_url`, for two reasons that both bite. A narrative chart's `queryParamsForParentChart` routinely carry `tab=chart`, and since a reference's params win the merge, the merged URL would open the target's line or slope view and you would build a replacement of the wrong view. And those params are the narrative chart's *"Explore the data"* href — where it sends readers to the parent — not the view it renders, which comes from its own materialized `configFull`. The old params still belong on the page, in their own column, because the control parents to the view on screen and nothing authored transfers: they are what you re-author the story from.
