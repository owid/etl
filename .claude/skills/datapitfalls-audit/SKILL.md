...
Use the wrapper `scripts/dp_scan.sh OUTDIR SLUG <scan-args…>` (it sources the key, runs `scan … --json --all`, validates JSON, prints a one-line summary). Default model is Sonnet 4.6 — don't pass a model flag unless the user asked. One scan per file:
```bash
...
The tool returns a **top-N (~8) findings per scan**, so large files that hit 8 likely hide more. Re-run the biggest 1–2 files on Opus and record the diff (it tends to be *more selective*, not deeper):
```bash
...
## Output convention
Everything goes under `ai/datapitfalls/`: `raw/*.json` (one per scan, kept), `charts/*.png` (scanned images), `text/*.txt` (prose inputs), `<short_name>_report.md` (the deliverable).
