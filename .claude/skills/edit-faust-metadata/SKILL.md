...
6. Apply the edit. **If the blast radius listed narrative-chart children, snapshot the parent's pre-edit rendered config FIRST** — save `chart_configs.full` for the parent chart from the staging DB (`SELECT cc.full FROM charts c JOIN chart_configs cc ON cc.id = c.configId WHERE c.id = <id>`) to a scratch file; the narrative-children pass compares against this, and after the edit the pre-edit text is gone. Then:
