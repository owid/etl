...
- **Screenshots of different frames or pages.** Issue them together, then download all the URLs in one parallel bash call (pattern and its two silent traps: Gotchas). A hosted screenshot is three tool calls and a run takes 14–70 of them — **locally, `scripts/figma_desktop_read.py` makes it one call at ~0.37 s instead of ~12 s**, when the file is the active tab; run its `check` first and fall back to the hosted path (Gotchas).
