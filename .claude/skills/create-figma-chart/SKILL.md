...
| Year stated, and not stale | the year the export actually shows — the link's `time=` where there is one, otherwise the rendered SVG — plus the source chart's `maxTime` | the image says which year it shows, and the source chart isn't pinned to an old year (`/check-hardcoded-years`) |
