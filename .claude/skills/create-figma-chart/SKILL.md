...
| Static Chart Template_Vertical | `5332:93` | 850×1095 | header `Frame 23`; footer **`Frame 25` (`25808:16`)** @ y=1015.81, h=63, same three rows. Header bands identical to the Horizontal at **118**. Note its header is **817.57** wide against the footer's **818** — a 0.43px mismatch in the template, so an ink-span check reports a 0.43px right-hand breach that is not yours |
...
**A header that hugs its text moves the band with the copy you write** — `primaryAxisSizingMode: "AUTO"`, with both the title and the subtitle at `textAutoResize: "HEIGHT"` and `layoutSizingVertical: "HUG"`. That is how the 540-wide set, both Instagram frames and DI ship. **The two 850-wide templates do not:** they arrive `FIXED` with a `layoutGrow: 1` child, so `headerSizing.reflows` comes back `false` on an untouched clone of either — that is their shipped state, not drift, and it is why the reflowed figures elsewhere in this file are quoted for the clone rather than the template. Make the clone hug (below) and a one-line title plus a one-line subtitle takes Static Vertical's `headerBottom` from the placeholder's 118 down to **70**.

Check every clone rather than assuming it, because the failure mode is silent and costs a lot of chart — and on the 850-wide pair it is the default. A header left at `FIXED` with a `FILL` / `layoutGrow: 1` child does **not** reflow: `headerBottom` stays at the placeholder's value however short the copy is, and the slack is absorbed by the flexible child's *box* instead — a one-line title leaves the subtitle's box 67px tall for 19px of ink, and fitting to the frame band then buries ~48px of dead air under the subtitle. Nothing renders wrong, so it survives a screenshot; the tell is a subtitle whose `height` is much larger than its line count justifies.
...
| Static Vertical (850×1095) | x=16, w=**817.57** (header) / 818 (footer) | **118 → 1015.81** |
...
**The two 850-wide rows hold only for a clone that has been made to hug.** Those templates ship `FIXED` with a `layoutGrow: 1` child (see the header rule above), so an untouched 850 clone does *not* reflow when you shorten the subtitle — it keeps `118` and inflates the subtitle's box instead. Set the three hug properties on the clone first, then the 99 applies. **The 99 itself is confirmed** — re-measured on the live templates 2026-08-20, the header rhythm is `origin_y 16` + a 29px title line + a 6px gap + a 19px subtitle line, so a two-line title with a one-line subtitle derives `16 + 58 + 6 + 19 = 99`, and a one-line title with a one-line subtitle derives 70. The hug is the precondition, not the number.
