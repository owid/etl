...
- **Data source:** `Data source: ` + `chart.citation` from Step 1 — that field *is* grapher's own footer line, so don't re-derive a `<producer> (<year>)` string by hand, and don't abbreviate it to save space. A long producer name overruns the CC BY text at x=468. **Writing this slot also collapses its weight — apply the mixed-weight restore below in the SAME call, rather than leaving Step 8c's `source-line-weight` row to find it: the whole line renders Bold, which reads as a slightly heavy footer rather than as a defect, and finding it late costs a check call plus a fix call.**
