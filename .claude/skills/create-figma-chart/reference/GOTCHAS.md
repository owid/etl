...
- **Never append a new auto-layout frame as a direct child of a template clone — group instead.** `figma.createAutoLayout()` is a real API and the `figma-use` skill recommends it for any container whose children relate to each other, which is good advice for building a design and wrong here: this skill fits content *into* a template it cloned, and the header and footer are resolved **structurally**, as the topmost and bottommost auto-layout children of the frame (`verify_page.js` → `const autos = …`). A third auto-layout child silently repoints `header` or `footer`, and with them the content box, the band top and the footer top — so `box-alignment`, `gap`, `margins` and `annotation-block-gap` all start reporting confident wrong *numbers* rather than an error. Bundle loose nodes with `figma.group(nodes, parent)`, which the structural resolver ignores.
- **A failed `use_figma` no longer promises it changed nothing — read `safeToRetryWithoutCanvasRead` off the error.** `true` means fix the script and retry; `false` means read the canvas and find out what landed before you write again. This replaced a flat guarantee ("failed scripts do not execute … no changes are made to the file") that Figma withdrew in plugin **2.2.107**, and their own bundle still contradicts itself — the old wording survives in the stale 2.2.96 cache and in `generate-project-plan`'s trap list. **Nobody has verified which is true of the Charts file's connector**, so treat a half-applied write as possible rather than impossible: the cost of assuming atomicity wrongly is a duplicate node you don't go looking for. What does not change is the discipline the old rule motivated — guard every property read in a mixed-type sweep, because one throw still costs the whole pass either way.
...
- **`entity-labels` children are not always TEXT.** When a bar's entity name wraps, grapher groups the two lines, so `node.fontSize = 14` throws `no such property 'fontSize' on GROUP` — and a throw costs you the whole pass. Iterate `group.query("TEXT")` for styling and `group.children` for per-row layout.
...
- **The Figma *desktop* MCP server serves reads ~30× faster, and is still the wrong tool for a chart
  ...
  renders **1–3 times** — 3 and 1 in the two runs that recorded the split (BENCHMARK.md) — and those
  are of frames it *just made*, which is precisely what this path cannot see (constraint 5). (An
  earlier version of this note said 14–70; that figure predates `measure_pixels.py` removing the
  reason most probes rendered at all, and sizing any screenshot work against it overstates it ~20×.) What is left saves a couple of minutes and buys a silent
...
- **A sweep over mixed node types must guard every property read.** `dashPattern`, `strokes` and `fills` don't exist on `GROUP`, so one un-guarded read aborts the whole script — and a throw costs you the entire pass, not just that node. Wrap each read in its own `try`, and remember `fontSize`/`fontName`/`lineHeight` can come back as `figma.mixed` rather than a value.
...
- **Figma deletes a group the moment it becomes empty, so never touch it afterwards.** Moving the last child out and then reading `group.children.length` throws `The node with id … does not exist` — and a throw costs you the whole script, not just that line. Drain the group and simply don't refer to it again.
