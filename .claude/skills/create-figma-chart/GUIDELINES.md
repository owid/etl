...
- **When a design choice knowingly fails a bar, record the deviation where the next person will read it.** Not everything gets fixed: a warmer color with better label contrast can be worth a colour-vision cost, and that is the author's call to make. What is not acceptable is leaving it silent, because the next run measures it, reads it as a bug and "fixes" it. Write the numbers, the reason and the alternatives that were measured — as a page note beside the frame (outside it, so it never lands in the exported PNG) and in the report. State the accepted figure explicitly; "we decided the colours were fine" is not a record.
