...
- **Every annotation goes in its own auto-layout frame, filled with the template's own canvas color, hugging the text on both axes.** White for the DI and static templates, `Instagram/Beige Background` `#FBF9F3` for the Instagram ones — read it off the template rather than hardcoding, because white on beige is a visible rectangle. That is the house mechanic and it is what the finished pages use: the frame keeps gridlines and context lines out of the letterforms, and because it hugs the text exactly and matches the canvas, it never reads as a box. Set the fill on the frame, `layoutSizingHorizontal`/`Vertical = "HUG"` on both the frame and the text, zero padding, and append the frame **last** so it sits above the chart. Hugging is the part that does the work — a frame left at a fixed size is the background rectangle the guidelines warn about, hiding chart detail on whichever side has slack.
  ...
  - **Set `leadingTrim = "CAP_HEIGHT"` on the text, or the knockout is bigger than it looks.** Hugging hugs the *line box*, not the ink, and the line box carries leading above the cap height and below the baseline: a two-line 14px label measured **34px tall for 27px of ink — 7px, 21%, of pure white padding**. On a map that surplus eats coastline for no reason. The trim removes it and hugging then fits the glyphs, so you get a tight footprint that still maintains itself. The box shrinks around the ink, so nudge `y` by half the height lost to keep the text where it was.
    - This is also why a designer will reach for "unset auto-layout and drag it smaller" — they are right that the frame is too big, but hand-sizing costs the self-maintenance: the frame stops tracking the text, and because `clipsContent` is `true` on a frame, a later edit to the value or the name gets **clipped rather than overflowing**, which is the kind of failure nobody notices. Trim the leading instead of unsetting the layout, and say so when you see fixed-size annotation frames in a file.
...
**There is a second, better-integrated set of values in the file, and where a named style exists it wins: context lines bound to `Data Insights/Axis Grid` `#DDDDDD` at 1.5px, their labels bound to `Data Insights/Axis Lines` `#999999`.** That is what the finished DI pages use, and it beats the raw pair below on the one axis that matters here — these are *named local styles*, so the muted layer stops being a pair of magic hex values and the Step 8c off-palette check stops having to carry an exception for it. It is also lighter (`#DDDDDD` is the gridline color, so context series sit at exactly the weight of the grid they cross) which suits a chart where the context is genuinely scenery. Prefer it whenever the protagonist is a saturated 3px line: measured against `#DDDDDD`, a Peach\* or Purple line at 3px reads instantly.

**The raw fallback, for when you want the context darker than the gridlines: lines `#a6a6a6` at 1px, labels `#8c8c8c`.** Reach for it when the context series still has to be *followed* rather than merely sensed — a reader comparing a dozen countries needs to trace one of the gray ones. Two things follow that a later run will otherwise try to "fix":
...
- **More than about four lines: mute all but the protagonist** — see "Highlighting one series against the rest". On a many-line chart this matters more than any other edit, and it is also what makes the direct labels legible, since a column of same-size labels in twelve different hues reads as noise.
  ...
  - **Expect grapher's own six-color assignment to fail the color-vision audit, and treat direct labelling as the mitigation.** Measured on `Camel* / Rusty Orange / Denim / Light Teal* / Purple / Maroon`: deuteranopia ΔE **9.2** (Denim vs Purple), 14.6 (Camel\* vs Rusty Orange), 17.7 (Light Teal\* vs Maroon) — three failing pairs, and the palette runs out at six categories however you choose them. What makes it survivable is that **no reader has to match a color to a key**: every line is named in place, so the colors distinguish rather than encode. Report the numbers, note the labelling, and leave the repaint to the chart's author.
