...
- **Small multiples take a 2px series line, not the house 3px.** Nine panels of 3px reads as heavy rule work rather than as data — the line stops being the thing that varies between panels and starts being the grid. The halo, where there is one, stays at line+1. `verify_page.js` gates this behind `CONFIG.faceted`, so declare it or every line reports as 1px too thin.
- **Don't give each facet its own background fill.** A panel is already defined by its own content — the entity name, the end value, the line, the year bracket — so a fill behind each one paints N tiles where the reader should see one chart, and it puts a second colour into a palette that otherwise holds only the series. Alignment does the grouping for free when the labels and axis brackets line up across columns. Some older DI pages use a cream panel per facet; treat that as a page to depart from rather than a convention to copy, and say so in the report when you do.
