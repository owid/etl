...
- **Set `clipsContent = false` on the frame.** Frames clip by default, and the `CAP_HEIGHT` trim below puts the baseline *at* the box bottom — so every descender is cut off: "today" renders "todav", "very" renders "verv". It survives a node listing and a quick thumbnail, and it is the single most embarrassing defect this mechanic can produce. Turn clipping off on every annotation frame; the knockout stays exactly as tight, the descenders simply overhang it.
