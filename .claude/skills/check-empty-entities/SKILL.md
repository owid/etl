...
Same rule as [`check-hardcoded-years`](../check-hardcoded-years/SKILL.md#always-close-with-whats-still-open) — end every run with an explicit open-items block, in the PR body as well as in chat, split into **handed off** (waiting on someone else — give them the locator, not just the finding), **proposed** (waiting on sign-off — state the exact change so "yes" is a complete answer), and **unverified** (fetch failures, unparsed explorer TSVs, surfaces skipped for cost, any top-N truncation). Unverified is the one that matters most here: a pre-existing gap left unmentioned reads as "clean". Re-state the whole block on every revisit, including after a round of fixes, rather than reporting only the delta.
