...
0b. **Check whether an automatic round is already in flight before you trigger one.** Marking a PR ready for review starts a Codex round on its own (~2.5 min to a verdict, measured), so a manual trigger posted right after `ready_for_review` adds a second round whose verdict is indistinguishable from the first's — a `+1` names no commit, so nothing attributes it. Read the ready timestamp first:
   ...
   gh api repos/<owner>/<repo>/issues/<n>/timeline --paginate \
     --jq '[.[] | select(.event=="ready_for_review") | .created_at] | .[-1] // "never a draft"'
   ```
...
- A reaction on the trigger comment is NOT the review, unless `content == "+1"` **from the Codex bot**. Codex may add a fast 👀-style acknowledgment reaction and submit the real review minutes later (~4–9 min), but the acknowledgment is not dependable — see the measured-zero-reactions lesson below. Exiting on any reaction declared "clean" and silently skipped two P2 findings (PR 6506). Use a review-count increase as the findings signal and a Codex-authored `+1` as the only clean-signal reaction; after any clean verdict cross-check the reviews count didn't also increase.
...
- **Don't expect the 👀 acknowledgment on the trigger comment, and read nothing into its absence.** Across four PRs that all drew real Codex reviews (6620, 6630, 6636, 6637), every one of their 15 `@codex review` comments carries zero reactions of any kind. An unreacted trigger is therefore the norm here, not evidence that Codex missed it, and never a reason to re-trigger — the clean-pass `+1` lands on the PR body instead.
