...
- **Worktree isolation is keyed per SESSION, not per agent — so once a babysitter has its own worktree, the main session must stay put.** Calling EnterWorktree on a *different* worktree re-keys the isolation both share, and from then on every Bash call the agent makes against its own tree is refused ("this command's working directory resolved to the shared checkout"), down to `pwd`. The agent cannot fix it from its side: EnterWorktree({path}) back to its own worktree reports success while the guard keeps reading the new one. Only the main session moving back clears it. So plan the run: if you need a second branch mid-loop (a follow-up PR, an unrelated fix), either wait for the babysitter to finish or accept that it is read-only until you return. Two behaviours make this survivable, and both belong in the agent's prompt — **keep polling read-only rather than stopping** (`gh api` calls don't touch the tree, so the loop stays alive and the round is not abandoned), and **report blocked rather than editing from the wrong tree**. Tell the agent explicitly when you're back, since it has no way to detect the guard clearing. (Cost one run several minutes: a follow-up PR opened mid-loop locked the babysitter out until the main session returned.)
