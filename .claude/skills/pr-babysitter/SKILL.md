...
4. **Fix valid findings**: `git pull --rebase` FIRST (the branch may have moved; if the shared tree is dirty with the main session's work, use `--autostash`). Use `.venv/bin/` for everything. Verify with the relevant `etlr` steps and `make check`. Commit `🐛🤖`/`🔨🤖` + "Co-Authored-By: Claude <model name> <noreply@anthropic.com>", push.
...
- Main session and agent share the checkout: both must `git pull --rebase` before committing, and the main session must not run a parallel review/CI loop. When the main session holds uncommitted user-review-pending work in the tree, two extra rules apply: (a) plain `git pull --rebase` refuses a dirty tree — the agent needs `--autostash`; (b) staging is file-level, so "stage only the files you edited (never `git add -A`)" is safe only for files the main session has NOT also touched — on a same-file collision, `git add <file>` would sweep the main session's unreviewed hunks into the agent's commit, so the agent must not commit that file at all: it reports the collision, freezes its commits, and the main session folds the agent's tree edits into the user-reviewed commit (coordinated via SendMessage). The clean way to sidestep both hazards is to hand the babysitter its own worktree of the branch.
