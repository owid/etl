...
- Main session and agent share the checkout: both must `git pull --rebase` before committing, and the main session must not run a parallel review/CI loop. When the main session holds uncommitted user-review-pending work in the tree, the agent must stage only the files it edited (never `git add -A`) — or better, the main session freezes the agent's commits via SendMessage and folds the agent's tree edits into the user-reviewed commit.
