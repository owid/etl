...
0b. **Check whether an automatic round is already in flight before you trigger one.** Marking a PR ready for review starts a Codex round on its own, so a manual trigger posted right after `ready_for_review` adds a second round whose verdict is indistinguishable from the first's — a `+1` names no commit, so nothing attributes it. Read the ready timestamp first:
   ...
   Note the shape, and keep it: `--paginate` applies `--jq` **once per page**, so an inline `--jq` prints one line per page and a caller reading the first or last line silently gets the wrong answer once a timeline exceeds one page. `--slurp` collects the pages into a single outer array, but `gh` rejects `--slurp` alongside `--jq` — hence the pipe to an external `jq`, and `.[][]` to flatten pages before selecting. The same trap applies to every `--paginate` call whose filter aggregates across pages.
...
- **`ready_for_review` starts its own Codex round**, which answers within a few minutes with no trigger comment anywhere. So triggering right after a PR is marked ready puts two rounds in flight, and a `+1` — naming no commit, with the 👀 already withdrawn — belongs to neither: that verdict is unattributable, and no amount of timing arithmetic recovers which trigger earned it. Read the ready timestamp before triggering (step 0b) and wait for that round instead of starting a second one. It does not fire on every PR — it appears tied to the PR author's own Codex connection.
