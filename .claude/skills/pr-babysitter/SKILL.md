...
0b. **Check whether an automatic round is already in flight before you trigger one.** A PR *becoming reviewable* starts a Codex round on its own, so a manual trigger posted just after that adds a second round whose verdict is indistinguishable from the first's — a `+1` names no commit, so nothing attributes it. Two of Codex's three documented triggers are outside your control: **opening a PR for review** and **marking a draft ready**. Both count, so find the moment the PR last became reviewable:
   ...
   ```bash
   gh api repos/<owner>/<repo>/pulls/<n> --jq '{created_at, draft}'   # opened ready?
   gh api repos/<owner>/<repo>/issues/<n>/timeline --paginate --slurp \
     | jq -r '[.[][] | select(.event=="ready_for_review") | .created_at] | .[-1] // "no readiness event"'
   ```

   The round started at the `ready_for_review` timestamp when there is one, and otherwise — no readiness event **and** `draft: false` — at the PR's own `created_at`, because it was opened ready and that is a trigger in its own right. **A PR opened non-draft never gets a readiness event**, so reading `no readiness event` as "no auto-round" inverts the newly-opened case, which is the one most likely to have a round in flight. Only `draft: true` with no readiness event rules an auto-round out.
...
- **A PR becoming reviewable starts its own Codex round**, which answers within a few minutes with no trigger comment anywhere. Both routes count — a draft marked ready, and a PR opened non-draft — and the second leaves no `ready_for_review` event to find, so a check that looks only for that event misses it. So triggering right after either puts two rounds in flight, and a `+1` — naming no commit, with the 👀 already withdrawn — belongs to neither: that verdict is unattributable, and no amount of timing arithmetic recovers which trigger earned it. Establish when the PR became reviewable before triggering (step 0b) and wait for that round instead of starting a second one. It does not fire on every PR — it appears tied to the PR author's own Codex connection.
