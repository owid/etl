...
0b. **Check whether an automatic round is already in flight before you trigger one.** A PR *becoming reviewable* starts a Codex round on its own, so a manual trigger posted just after that adds a second round whose verdict is indistinguishable from the first's — a `+1` names no commit, so nothing attributes it. Two of Codex's three documented triggers are outside your control: **opening a PR for review** and **marking a draft ready**. Both count, so find the moment the PR last became reviewable:

   ```bash
   gh api repos/<owner>/<repo>/pulls/<n> --jq '{created_at, draft}'   # current state, not the opening one
   gh api repos/<owner>/<repo>/issues/<n>/timeline --paginate --slurp \
     | jq -r '[.[][] | select(.event=="ready_for_review" or .event=="convert_to_draft")
              | "\(.event) \(.created_at)"] | if length==0 then "no draft transitions" else .[] end'
   ```

   The round started at the **last** `ready_for_review` timestamp when there is one. With no readiness event it started at the PR's own `created_at`, provided the PR was ever reviewable — true when `draft: false`, and equally true when a `convert_to_draft` event exists, since a PR can only be converted *from* a reviewable state and without a readiness event the only such state is the one it was opened in. **A PR opened non-draft never gets a readiness event**, so reading `no readiness event` as "no auto-round" inverts the newly-opened case, which is the one most likely to have a round in flight — and the current `draft` flag is not the opening state, so it cannot stand in for one: opened-ready-then-converted-back is a routine few-minutes-later correction (PR 5211 did it 14 minutes in), and it reads `draft: true` with no readiness event while a round that started at creation may still be answering. Only a PR that has been a draft since creation — no readiness event, no draft conversion, `draft: true` — rules an auto-round out.
...
- **A PR becoming reviewable starts its own Codex round**, which answers within a few minutes with no trigger comment anywhere. Both routes count — a draft marked ready, and a PR opened non-draft — and the second leaves no `ready_for_review` event to find, so a check that looks only for that event misses it, and one that falls back to the current `draft` flag misses it again on a PR opened ready and later converted back. So triggering right after either puts two rounds in flight, and a `+1` — naming no commit, with the 👀 already withdrawn — belongs to neither: that verdict is unattributable, and no amount of timing arithmetic recovers which trigger earned it. Establish when the PR became reviewable before triggering (step 0b) and wait for that round instead of starting a second one. It does not fire on every PR — it appears tied to the PR author's own Codex connection.
