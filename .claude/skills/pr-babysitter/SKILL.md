...
- **A `+1` verdict must be authored by the Codex bot, not just well-timed.** Humans routinely 👍 a PR description; content + timestamp alone would read that as a clean pass, and the reviews-count cross-check can't save you while the real findings review is still in flight (it hasn't been submitted yet, so the count hasn't moved). Always check the reaction's `user.login` (`chatgpt-codex-connector`) before treating any `+1` as a verdict, in both reaction locations.
