...
0b. **Check whether an automatic round is already in flight before you trigger one.** Marking a PR ready for review starts a Codex round on its own (~2.5 min to a verdict, measured), so a manual trigger posted right after `ready_for_review` adds a second round whose verdict is indistinguishable from the first's — a `+1` names no commit, so nothing attributes it. Read the ready timestamp first:

   ```bash
   gh api repos/<owner>/<repo>/issues/<n>/timeline --paginate --slurp \
     | jq -r '[.[][] | select(.event=="ready_for_review") | .created_at] | .[-1] // "never a draft"'
   ```

   Note the shape: `--paginate` applies `--jq` **once per page**, so the inline-`--jq` form prints one line per page — a bare `never a draft` for every page without a readiness event — and a caller reading the first or last line gets the wrong answer on any PR whose timeline exceeds one page (`--paginate` requests 100 events per page, so it takes a heavily-reviewed PR to get there, and the failure is silent when it does). `--slurp` collects the pages into one outer array, but `gh` rejects it alongside `--jq`, so the filter has to run in an external `jq` — hence the pipe, and `.[][]` to flatten pages before selecting. Verified to print exactly one line on a multi-page timeline, a single-page one, and a PR that was never a draft.

   If the PR was marked ready within the last few minutes, **wait for that round instead of triggering** — step 0's rules apply to it exactly as to one you posted. Trigger only when no auto-round can be answering, and pass the ready timestamp to the agent either way so it can tell the two apart.

   **Waiting for an auto-round means adapting the template, which assumes a trigger you posted.** Its opening line asserts an `@codex review` comment and step 2 reads the clean-pass `+1` from that comment's id — neither exists for a `ready_for_review` round, and an agent handed the template unchanged will either invent a comment id or skip a surface it was told was required. So when you hand off an auto-round, say so explicitly in the prompt: the polling threshold is the **ready timestamp** instead of a trigger timestamp, there is no trigger comment, so the trigger-comment reaction check is dropped and the `+1` is watched on the **PR body only** (`issues/<n>/reactions`) alongside the three review surfaces, and the round covers the head SHA as of the ready event. Tell it not to re-trigger on that round's verdict without your go-ahead — posting `@codex review` converts the auto-round into the ambiguous two-round case this step exists to avoid.
...
- **`ready_for_review` starts its own Codex round, so a fast verdict may not be answering your trigger at all.** Measured with NO `@codex review` comment anywhere on the PR: 6624 got a body `+1` 2m46s after being marked ready, and 6625 a full review 2m31s after. So when a PR is marked ready around the time you trigger, two rounds are in flight — and a `+1`, carrying no `Reviewed commit:` line, cannot be attributed to either. On 6637 the `+1` landed 1m52s after ready and 81s after the trigger — quicker than either round's observed band, and with nothing whatsoever to separate them: a `+1` names no commit, and the 👀 that would have shown which round was live is withdrawn as the verdict lands. Treat that round as unattributable rather than guessing a winner; the point of the check below is to avoid creating the ambiguity, not to resolve it afterwards. So record the `ready_for_review` timestamp alongside the trigger's, as a second possible round start, and never report a `+1` as *your* round's verdict when an auto-round could have produced it. The auto-round appears tied to the PR author's own Codex connection — PRs opened by other authors in the same window drew no verdict at all — so don't assume it fires on every PR.
- **The 👀 acknowledgment is transient — it marks a round in flight, and Codex withdraws it when the verdict lands.** Measured live on 6638: 12 seconds after the trigger, 👀 was present on both the trigger comment *and* the PR body; by the time the findings review was submitted 2m55s later, both were gone. That resolves an earlier post-hoc measurement that read as "Codex never reacts" — across four PRs with real reviews (6620, 6630, 6636, 6637), all 15 `@codex review` comments showed zero reactions, because every one of those rounds had already finished when they were counted. So an unreacted trigger is not evidence Codex missed it and never a reason to re-trigger, and a *present* 👀 is a useful positive signal that the round started — just never a verdict. Only a Codex `+1`, an issue comment, or a new review is that.
- **A manual `@codex review` is answered on a draft PR.** Measured on 6638: still a draft, never once marked ready, trigger at 16:31:19Z, 👀 at 16:31:31Z, full findings review at 16:34:14Z. So draft state is not a reason a trigger would go unanswered — don't let an agent report "no verdict, probably because it's a draft", and don't mark a PR ready to coax a review out of it (that starts a second round, per step 0b).
