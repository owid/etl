...
- **Manual-upload snapshots: also re-check for a stable download endpoint.** Distinct from the inverse-UA case above: producers add direct links over time. The WB Data Catalog now exposes stable date-less endpoints (`https://databank.worldbank.org/data/download/<PRODUCT>_CSV.zip`), which turned WWBI's manual `--path-to-file` snapshot into a script-less `url_download` `.dvc`. When the endpoint serves a multi-file zip, snapshot the zip itself and read the member file in meadow via `with snap.extracted() as archive: tb = archive.read("<file>.csv")` — still script-less. Bonus: bundled codebook/series files (e.g. `WWBISeries.csv`) can be passed through meadow as a second table and attached in garden as per-indicator `description_from_producer` — inventory the file's fill rates and distinct values first, and skip fields owned elsewhere (units → `unit`/`short_unit`, license → origin license, dataset-level boilerplate → origin description).
- **Audit blanket title-based unit scaling per indicator before trusting it.** A garden rule like "multiply every indicator whose title contains 'share' or 'percentage' by 100" assumes one storage convention across the whole file — but sources mix conventions. Real case (WWBI): the 134 "share"-titled indicators are fractions (raw max exactly 1.0), while the 2 "percentage"-titled wage-bill ratios are already percent — they come from a different upstream source (IMF FAD, revealed by the series file's `Source` column) — so the old step shipped them 100× too large for years (France's wage bill read 1,242% of GDP). Scan each scaled indicator's raw min/max, split the rule by observed convention, and add sanity checks on both sides of the scaling (raw fractions ≤ 1; scaled percentages within a grounded bound). **"Matches the previous version" is not evidence of correctness** — a magnitude bug can be inherited from the old step; check absolute plausibility against real-world values, not just old-vs-new equality.
