...
7) Indicator upgrade (optional, staging only)
   ...
   - **Both step-7 audits below share one surface sweep.** `find-chart-references` enumerates the surfaces carrying the dataset's indicators and classifies each reference as `embed` / `render` / `link`. Run it once — `--dataset-id <new grapher dataset id> --transitive --json ai/refs.json` (`--json` takes the output path) — and let both audits read that list instead of each re-deriving its own joins. From a dataset subject it covers charts, MDim views, chart-parented narrative charts, the article links/embeds and data insights of those charts, plus explorers *aggregated per explorer*. It does **not** cover per-view explorer configs, `linkType='url'` article rows, or anything hanging off an MDim/explorer (their article links, and narrative charts pinned to an MDim view) — so follow up with a second pass, `--mdim <slug>` / `--explorer <slug>` for the MDims and explorers the first run returned, and let each audit keep its own `explorer_views` join. It is also the tool to reach for outside these audits, whenever the question is "what would this change touch".
