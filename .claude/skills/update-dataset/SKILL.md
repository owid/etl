...
**Size the fan-out first** (`--dry-run` lists the scope). Small (≲50 steps): run the build check as written. Foundational-dataset scale (hundreds of steps — an income-groups bump is ~560): still worth running, but as a **one-time gate before the final push**, not something to iterate on — at that scale it costs ~35 min and ~7 GB of `data/` (unchanged upstreams download from the catalog instead of rebuilding), so delete the builds afterwards if disk is tight (`data/` is regenerable cache). And skip the **local** `etl diff` entirely at that scale — another ~25 min and a JSON in the hundreds of MB; owidbot's hosted report on the PR is the same comparison for free.
...
**The must-have trap (real case: `population` + `unaids`, FY2027 income groups).** Steps passing `countries_that_must_have_data` pin members to a group (`"Lower-middle-income countries": [..., "Philippines", "Vietnam"]`). When the producer reclassifies a pinned country out of its group, the **entire aggregate nulls for all years** — a total-entity wipe, not edge churn. Two nets catch it: `geo.add_region_aggregates` raises `ValueError` at build time (this PR's guard — so the build check above fails loudly), and probe 5 is the backstop. The fix is updating the must-have list to the new membership — so whenever bumping income groups, grep the repointed consumers for `countries_that_must_have_data` up front. Beware the rationalization risk: a wiped income-group aggregate *looks like* the expected churn the rest of the report is full of, and on a foundational dataset (population) it also announces itself as a dataset-count explosion — hundreds of consumers suddenly changed. Run probe 5 on every loss, however plausible the churn story.
