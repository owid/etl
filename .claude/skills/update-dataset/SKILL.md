...
- [ ] Empty-entity audit after all remaps: charts, map `columnSlug`, MDim views, narrative charts, and gdoc `country=` references — every pinned selection must intersect its variables' entities-with-data; grade findings against production (regression = fix, pre-existing = document; see step 7)
...
7) Indicator upgrade (optional, staging only)
   ...
   - **Empty-entity audit — after all remaps, verify every view's entity selection still has data.** An upgrade can leave a view pointing at a variable that lacks data for the entities the view pins — it renders as an empty chart with no error anywhere. Audit every surface that stores an entity selection against the entities that actually have data in the new indicators:
     - **Availability lookup:** a variable's entities-with-data come from the staging indicators API — `https://api-staging.owid.io/staging-site-<branch>/v1/indicators/<id>.metadata.json` → `dimensions.entities.values[].name`. Cache per variable id; fetch in parallel (the audit touches hundreds of variables).
     - **Charts:** for every chart on the new dataset (`chart_dimensions` → `variables.datasetId`), parse `chart_configs.full` → `selectedEntityNames` + y-dimension `variableId`s; flag any chart whose selection intersects none of its y-variables' entities. Skip ScatterPlot/Marimekko — they legitimately have no `selectedEntityNames` (they plot all entities). A missing selection on other chart types is only a finding if production's config differs (the upgrader never touches entity selections, so an empty selection is usually pre-existing — verify via Datasette before flagging).
     - **Map tabs:** for every chart with `hasMapTab`, assert `map.columnSlug` is one of the chart's dimension variable ids (it's stored as a *string* — str-cast before comparing; a stale one silently pins the map to the old variable, the #6457 bug class).
     - **MDim views** (`multi_dim_x_chart_configs` → `chart_configs`) **and narrative charts** (`narrative_charts.chartConfigId` → `chart_configs`): same selection-vs-availability check on their configs.
     - **Article references:** `posts_gdocs_links` rows (`linkType='grapher'`, published gdocs only) whose `queryString` carries `country=` pin entities in the URL — the upgrader never rewrites these. Parse tokens: `~`-separated; legacy URLs use `+` (decodes to spaces, so try greedy multi-word entity-name matches); skip `$entityCode` template placeholders; resolve ISO codes via the `entities` table; match `target` against `chart_slug_redirects` too. Only a **fully dead selection** matters — the link opens an empty chart; a partial gap still renders the remaining entities. To hand findings to content people, link each citation with a scroll-to-highlight URL (reuse `find_chart_citations_in_content` from `apps/wizard/app_pages/chart_diff/citations.py`; note its embedded-chart pass only scans top-level body blocks, so recurse for charts nested in layout containers).
     - **Classify against production before reporting:** fetch the same variables' entity lists from `https://api.ourworldindata.org/v1/indicators/<id>.metadata.json` (production variable ids via public Datasette, which covers only ~80% of chart ids — say so when a chart has no baseline). A selection that had data on production and lost it on staging is a **regression from this update — fix it before merge** (remap the view or restore the entities). A gap identical on production is pre-existing: list it in the PR body for content follow-up, don't block. (WDI 2026-07 run: 480 charts, 37 MDim views, 18 narrative charts, 585 gdoc references audited — zero regressions, 10 pre-existing gaps.)
