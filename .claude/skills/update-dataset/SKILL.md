...
7) Indicator upgrade (optional, staging only)
   ...
   - **Editing a parent chart's FAUST does not immediately reach its narrative children.** A narrative chart's `chart_configs.full` is materialized; after a parent edit it can keep serving the old inherited text (stored config *and* render) even though the field isn't pinned in the child's patch. Re-save the child's patch unchanged (`AdminAPI.update_narrative_chart(id, patch)`) to force re-derivation, or re-check later — never conclude from an immediate read that the parent edit didn't take.
