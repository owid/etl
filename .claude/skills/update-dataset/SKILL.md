...
9b) Data update post (for OWID /latest)
   ...
   **The skill's job is to produce paste-ready Google Doc content** in the exact CMS format the team uses (frontmatter `title` / `excerpt` / `type` / `authors` / `kicker` → `\[+body\]` marker → body prose with inline markdown links → `{.cta}` block → `{.image}` block → `\[\]` end marker). Don't invent your own format — every published post in the Drive folder follows the same shape.
   ...
   - **Create the Google Doc directly** in the `/Data updates` folder so the user doesn't have to copy-paste, using the Drive MCP `create_file` tool:
     `create_file(title="<YYYY-MM> Data update: <Name>", parentId="1oL0uLHKI6f2qi1rJA6-qFFRYEBw_-rfm", contentMimeType="text/markdown", textContent=<laid-out draft>)`.
     The default markdown→Doc conversion turns the body's `[text](url)` into **real hyperlinks** and preserves the `\[+body\]` / `{.cta}` / `{.image}` markers as literal text. Match the folder's title convention (e.g. "2026-06 Data update: Homicides").
     - **Target layout = compact, no empty paragraphs.** The team's docs put every line on its own paragraph with *nothing* blank between them (frontmatter, `\[+body\]`, body paragraphs, `{.cta}`/`{.image}` markers all consecutive) — Google Docs uses one paragraph per Enter, so blank "spacer" lines are wrong. Do **not** double-space the upload (`"\n\n".join(...)` inserts an empty paragraph between every line, which then has to be deleted). Upload the draft with single newlines; if the markdown importer merges lines that should stay separate, finalize with the add-on below.
     - **Use the OWID GDocs Add-on to format/finalize** (Extensions → OWID add-on in the Doc). It's the team's tool for getting these CMS docs into the right shape (paragraphs, links, markers) — rely on it rather than hand-tuning spacing through the upload.
     - **No raw 4-byte emoji** (e.g. 👉) in `textContent` — they mojibake through the upload (`ð`). Use plain text in the `:skip` pre-publish reminder (the user restores the emoji in the Doc if they want it).
     - The MCP has **no delete or edit-content tool**, so get it right on the first `create_file` — then `read_file_content` to verify. A bad create leaves an orphan Doc the user must delete manually; don't recreate on a trivial issue (tell the user the one-line fix instead).
     - If the shared folder rejects the write, create in My Drive and share the link.
   - At the end of the update, tell the user with **markdown links** to both the created Doc and the local draft: `"Data update post created at [<Doc title>](<doc viewUrl>) in /Data updates — please review, complete the `:skip` reminder, add the chart screenshot, and share. Draft source: [workbench/<short_name>/data-update.md](workbench/<short_name>/data-update.md)."`
