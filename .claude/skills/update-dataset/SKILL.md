...
9) Slack announcement & PR update
   ...
   - **Add the announcement to the PR description** as a collapsed `<details>` section titled "Slack announcement". Embed the content as **rendered markdown, NOT inside a code fence** — a ```` ```markdown ```` block forces horizontal scrolling for long lines and is awkward to copy, whereas rendered markdown wraps and is easy to read and select. Drop the draft's top matter (the `# …` title, `Source: …` line, and leading `---`) and embed from the first `## ` field heading onward (the `## field` + prose structure already renders and wraps cleanly). Put a blank line after `</summary>` so GitHub renders the markdown.
...
9b) Data update post (for OWID /latest)
   ...
   **The skill's job is to produce paste-ready Google Doc content** in the exact CMS format the team uses (frontmatter `title` / `excerpt` / `type` / `authors` / `kicker` → `\[+body\]` marker → body prose with inline markdown links → `{.cta}` block → `{.image}` block → `\[\]` end marker). Don't invent your own format — every published post in the Drive folder follows the same shape.
   ...
   - **Add a collapsed `<details>` section titled "Data update post (for OWID /latest)"** to the PR description, placed *after* the Slack-announcement section. Embed the content as **rendered markdown, NOT inside a code fence** (same reason as the Slack section — code fences force horizontal scroll and are hard to copy). Because the CMS format relies on line breaks (frontmatter fields, `\[+body\]` / `{.cta}` / `{.image}` markers) that markdown would otherwise collapse, lay it out with **one blank line between every non-blank line** — e.g. `"\n\n".join(l for l in draft.splitlines() if l.strip())` — so each renders as its own wrapping paragraph. Body paragraphs keep their inline `[text](url)` links (they render as clickable links). Put a blank line after `</summary>`.
