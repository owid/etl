...
>
> **The two announcements are owned elsewhere.** [`/data-updates-comms`](../data-updates-comms/SKILL.md) owns the internal Slack form (step 9) and [`/data-update-announcement`](../data-update-announcement/SKILL.md) owns the public `/latest` post (step 9b). Both steps here are delegation stubs on purpose — put wording, format, and style guidance in those skills, never back into this file.
...
- [ ] Run the referencing-prose audit (standard) — read the prose of every surface the sweep found (articles, data insights, key-chart blurbs) for quantitative claims this update invalidates; an unbounded claim ("has grown 1,300-fold", "now accounts for X%") goes stale, a time-bounded one ("by late 2025 it had reached $62B") does not. Hand the recomputed numbers to content — never edit published prose yourself (see step 7)
...
7) Indicator upgrade (optional, staging only)
   ...
   - **Referencing-prose audit (standard — run after all remaps).** The other two audits check whether a
     ...
     forget. Take the surfaces the sweep already found and read their text (`posts_gdocs.markdown`, joined via
     `posts_gdocs_links.target`), then re-derive every quantitative claim against the new data.
     ...
     **A static image changes the remedy, not the finding.** Use the sweep's classification: a surface that
     `embed`s a live chart picks the new data up on its own, so only its prose can drift. A surface presenting
     a *static image* has prose and picture that must agree — editing one sentence there desyncs them and is
     worse than leaving both consistent-but-dated, so the honest options are "refresh image and text together"
     (a separate static-viz job) or "leave it, and record why". Say which you are recommending.
     ...
     left, with the reason) in the PR's open items, so the next updater doesn't re-flag it.
     (NVIDIA Q2 FY2027: a data insight titled "…has grown 1300-fold in the last 12 years" was accurate at
     publication — 1,320x — and the new quarter took it to 1,562x. A sibling announcement post looked stale by
     the same arithmetic but was explicitly bounded to "late 2025" and needed nothing; the data insight was
     left unchanged because its chart is a static image.)
...
**Close every hand-off with an explicit list of what's still open — in the PR body, not just chat** (chat scrolls away; the PR is what the reviewer and future-you actually read). This is one of the two cases where the list is worth writing out formally rather than as a sentence; see `.claude/docs/open-items.md` for what tends to get dropped. An update generates far more loose ends than it closes; this workflow's usual danglers: content/gdoc edits and producer error reports (handed off), reader-facing config changes held for sign-off (awaiting a decision), audits offered but not run and anything the staging build didn't cover (nobody checked it) — plus a **fourth category, deferred to a follow-up PR**, where the two-PR pattern is in play (downstream consumers to repoint, old-version archiving, charts on retired indicators): that list is the follow-up PR's scope, so losing it means redoing the analysis.
...
- **Derive 52/53-week fiscal period ends from the year-end anchor, never from a per-month calendar rule.** When a producer states a "52/53-week period ending on the last <weekday> of <month>" calendar, its interior quarters are 13-week blocks counted from the previous year end — which is *not* the same as "the last <weekday> of the quarter's month". The two rules agree most years and diverge by a full week in the year after a 53-week year, when the 13-week step lands in the following calendar month. The final quarter is the exception: it ends on the fiscal year end itself and absorbs the 53rd week, so the per-month rule is correct there. This bug is invisible to every schema, bound and sum check — the values are all right, only the dates they sit on move — so pin the derivation with a handful of period-end dates quoted from the producer's own releases and assert against them, the same way a codebook's worked examples are used as test vectors. (NVIDIA: "last Sunday of Apr/Jul/Oct/Jan" dated 4 of 50 quarters seven days early, verified wrong against NVIDIA's own "quarter ended August 1, 2021" wording.)
...
- **Unguessable producer filenames: find the document index instead of brute-forcing URLs.** When a producer's download filenames follow no stable convention, stop guessing after one or two 404s and look for the machine-readable index behind their JavaScript-rendered page — the page HTML itself is usually an empty shell, so `curl`/WebFetch on it finds nothing. Q4 Inc investor sites (very common for US-listed companies) expose `/feed/<Service>.svc/<Method>?...&Year=<y>` returning JSON with one entry per document, tagged by category. Record the endpoint as a `# NOTE:` in the snapshot so the next updater looks it up rather than repeating the search, and note that the same index answers "was any older file moved or replaced" — comparing it against the hardcoded URLs is a cheap in-place-revision check. (NVIDIA renamed its quarterly PDF to `Q227-Revenue-by-Market-Platform-Slides.pdf` where the previous eleven quarters implied `Rev_by_Mkt_Qtrly_Trend_Q227.pdf`; six guessed patterns all 404'd.)
