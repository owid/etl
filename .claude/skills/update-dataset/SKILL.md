...
7) Indicator upgrade (optional, staging only)
   ...
   - **The mapping only covers `old_version → new_version`; charts stranded on *older* versions stay stranded.** Previous cycles leave charts behind, and the upgrader cannot see them. After the main upgrade, sweep **every** version of the dataset, not just the one you bumped from:
     ```sql
     SELECT d.catalogPath, COUNT(DISTINCT c.id) FROM charts c
     JOIN chart_dimensions cd ON cd.chartId=c.id JOIN variables v ON cd.variableId=v.id
     JOIN datasets d ON d.id=v.datasetId
     WHERE d.catalogPath LIKE '<namespace>/%/<short_name>' AND c.publishedAt IS NOT NULL GROUP BY 1
     ```
     **Scope the predicate to the namespace, and escape literal underscores in the pattern** (`LIKE '<namespace>/%/fertility\_rate'`) — short names recur across namespaces (`fertility_rate` exists under both `demography` and `gapminder`), a suffix-only `LIKE '%<short_name>'` sweeps unrelated datasets into the results, and an unescaped `_` is a single-character wildcard in MySQL `LIKE`, so it can quietly match a similarly named dataset too; since the follow-on mapping pairs variables by `shortName` alone, either slip would redirect another dataset's charts onto your new variables. Anything on an older version is remappable the same way (one `WizardDB.add_variable_mapping` per old dataset id, paired by `shortName`). This matters beyond tidiness: the flagship chart for the very indicator an update exists to fix can be sitting on a version from months earlier and receive none of the new data. Whatever remains after the sweep is charts whose indicators the producer **retired** — no `shortName` twin exists, so they need a replace-or-retire decision and are the reason those old versions can't be archived. (WDI 2026-07: 27 charts recovered across three older versions, including `gdp-per-capita-worldbank` at ~455 views/day, stranded five months on `2026-02-27`; 8 remained on retired indicators.)
...
**`etl approve` needs a production connection, and the tunnel it expects is usually down.** It compares staging against production, so `ENV_FILE_PROD` must be set (`ENV_FILE_PROD=.env.live .venv/bin/etl approve …`) — without it the command asserts immediately. `.env.live` points at `127.0.0.1:3310`, an SSH tunnel that is often not running; the reliable path is Tailscale, where the production grapher DB is reachable directly as host `prod-db` on port 3306 with the same `live_grapher` credentials (the pattern `add-ivs-indicators/scripts/indicator_admin_table.py` already uses, and worth making permanent in `.env.live`). Note the approval writes to the *staging* chart-diff table and only reads production, so the prod connection is read-only.
