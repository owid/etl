...
7) Indicator upgrade (optional, staging only)
   ...
   - **Referencing-prose audit (standard — run after all remaps).** The other two audits check whether a
     ...
     **A static image changes the remedy, not the finding — and the sweep's `kind` cannot tell you which you
     ...
     publication — 1,320x — and the new quarter took it to 1,562x. A sibling announcement looked stale by the
     same arithmetic but was explicitly bounded to "late 2025" and needed nothing. Both showed static images,
     both were `kind=embed`, and a hand-rolled `posts_gdocs_links` sweep had found 4 references where the
     script found 6 — the missing two being the data insight itself and a featured metric.)
