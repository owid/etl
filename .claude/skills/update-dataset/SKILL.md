...
- [ ] Empty-entity audit after all remaps: run the `check-empty-entities` skill (charts, map `columnSlug`, MDim views, explorer views, narrative charts, gdoc `country=` references); regressions vs production = fix, pre-existing = document (see step 7)
...
5) Garden step repair/verify (step-fixer subagent, channel=garden)
   ...
   **Long-format tables hide additions from the column diff.** When a table keys its series by a dimension column (`indicator`, `metric`, `question`, …) with a stable `value` column, a new series adds *rows*, not columns — the column-set diff stays empty. Also diff the distinct **combinations** of the identifying dimensions, not each dimension's values separately — a new series can be a new tuple of existing values (e.g. `sex=male` and `metric=count` both existed before, but the `(male, count)` series is new): `set(tb_new[dims].itertuples(index=False, name=None)) - set(tb_old[dims].itertuples(index=False, name=None))` where `dims` is the list of identifying dimension columns (with a single dimension this reduces to a value-set diff). The grapher step expands dimension values into separate variables, so the step-7 shortName pairing surfaces these too — but catch them here, before staging.
