...
**Dod links do not survive the HTML import.** A fragment href like `#dod:oda` gets rewritten by Google's importer into an internal bookmark id (`#id.xxxxxxxx`), silently breaking the dod. External `https://` links come through fine (wrapped in Google's redirector, which is normal). After creating the doc, list every dod the body uses in the handoff and ask the user to re-add each one by hand (select the anchor text → edit link → paste the literal `#dod:…`) — a manual re-add through the Docs UI **does** store the bare fragment, which is what the CMS needs (dod detection matches `url.startsWith("#dod:")`; see `MarkdownTextWrap.tsx` / `SimpleMarkdownText.tsx` in owid-grapher, and existing article gdocs carry bare `#dod:…` hrefs). Verify the fix with `download_file_content(exportMimeType="text/html")` and grep for `href="#dod:…"` — do **not** judge it from `read_file_content`, whose markdown rendering resolves the fragment against the doc's own URL and makes a correct link look broken.
...
**Numbers must survive the click-through.** Every figure and ranking in the body must match what the *linked OWID charts* show. Producers often headline a different price basis or measure than our charts carry (current vs constant prices; grant equivalents vs net) — a producer-basis claim ("Germany overtook the US", "$174 billion") that our charts contradict invites every careful reader to catch the mismatch. Either attribute such claims explicitly to the producer, make the number basis-robust (round it so both bases agree: "around $170 billion", "roughly a quarter of a percent"), or — simplest — write about our charts only. Enumerate the measure × price-basis combinations before echoing any producer ranking; it may hold on only one of them. (ODA 2026-07: "Germany largest donor" held only on grant equivalents at current prices, by 0.5%; every OWID chart showed the US ahead.)
