...
This review can burn a lot of tokens (full pipeline run, link checks, several metadata-quality skills, optionally the adversarial review). If the user wants to know how long or how much a past review cost, use the [`cost-report`](../cost-report/SKILL.md) skill afterwards — it reconstructs the total from session transcripts, so nothing needs to be logged live here.
