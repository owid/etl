...
### 10b. Adversarial data review (verify it ran, then spot-check)

`/update-dataset` § 6c-bis requires an adversarial factual review via [`/adversarial-data-review`](../adversarial-data-review/SKILL.md). As reviewer, verify the **outcome**: the PR body (or `workbench/<short_name>/update-context.yml`) references an `ai/adversarial-review-<short_name>-<date>.md` report, and its 🔴 findings are resolved — metadata edited, or a `<short_name>.corrections.yml` added with `reason`/`provider`/`status` filled in. A missing report on a substantive update (new indicators, changed methodology, editorial claims) is 🟡.

Then spot-check independently — don't take the report's word for it: re-verify 2–3 of its findings and 2–3 anchor values (World total + one major country, latest year) against an independent source yourself, following that skill's independence rules (a different producer measuring the same quantity; never OWID republishers or mirrors of the same producer). A value you confirm wrong that the report missed or waved through is 🔴.
