...
1. **Downstream builds:** on the PR branch, `etlr --modified --continue-on-failure --private --dry-run` lists every affected step (changed vs master + full transitive downstream); drop `--dry-run` to rebuild them and surface any that crash against the new dependency. Staging hides this failure mode — a failed step just stays stale in the catalog, so data-diff shows it as unchanged.
2. **Value changes:** read owidbot's **data-diff** comment + HTML report (`https://catalog.ourworldindata.org/diffs/<branch>/data-diff.html`) — staging (new dep) vs production (old dep), per-column changed-value counts with old→new samples. Scan for the silent-drop signatures: a column/entity that vanished, a series gone all-NaN, region-aggregate values that collapsed. Locally: `etl diff REMOTE data/ --changed --include garden --output-html data-diff.html`.
