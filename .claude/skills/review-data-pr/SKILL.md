...
- **Slack + `/latest` drafts are not expected in the PR body at all.** `/update-dataset` keeps them in the author's `workbench/` (steps 9 / 9b, owned by `/data-updates-comms` and `/owid-staff:draft-data-update-post`), so their absence from the PR is correct — don't flag it.
