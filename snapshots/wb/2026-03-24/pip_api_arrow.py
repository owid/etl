...
NOTE: This is the Arrow/Feather version of pip_api.py. All intermediate cache files use Feather format
for ~5-14x faster I/O. Final output files (world_bank_pip.csv, world_bank_pip_percentiles.csv,
world_bank_pip_regions.csv) are still CSV. Cache directories from the CSV version are NOT compatible —
delete .cache/ before running this version.
...
Copy these files to this folder and run in the terminal:
    ...
    etls wb/{version}/world_bank_pip_regions --path-to-file snapshots/wb/{version}/world_bank_pip_regions.csv
...
def pip_query_country(
    ...
    if download == "true":
        ...
        # Save to feather
        df.to_feather(
            f"{CACHE_DIR}/pip_country_data/pip_country_{country_code}_year_{year}_{popshare_or_povline}_{int(round(value * 100))}_welfare_{welfare_type}_rep_{reporting_level}_fillgaps_{fill_gaps}_ppp_{ppp_version}.feather",
        )
...
def pip_query_region(
    ...
    if download == "true":
        ...
        # Save to feather
        df.to_feather(
            f"{CACHE_DIR}/pip_region_data/pip_region_{country_code}_year_{year}_{popshare_or_povline}_{int(round(value * 100))}_ppp_{ppp_version}.feather",
        )
...
def generate_percentiles_raw(wb_api: WB_API):
    ...
    def get_percentiles_data(povline, versions, ppp_version, country_code):
        ...
        """
        if Path(
            f"{CACHE_DIR}/pip_country_data/pip_country_{country_code}_year_all_povline_{povline}_welfare_all_rep_all_fillgaps_{FILL_GAPS}_ppp_{ppp_version}.feather"
        ).is_file():
            return
    ...
    def get_percentiles_data_region(povline, versions, ppp_version):
        ...
        """
        if Path(
            f"{CACHE_DIR}/pip_region_data/pip_region_all_year_all_povline_{povline}_ppp_{ppp_version}.feather"
        ).is_file():
            return
        else:
    ...
    def get_query_country(povline, ppp_version, country_code):
        ...
        file_path_country = f"{CACHE_DIR}/pip_country_data/pip_country_{country_code}_year_all_povline_{povline}_welfare_all_rep_all_fillgaps_{FILL_GAPS}_ppp_{ppp_version}.feather"
        if Path(file_path_country).is_file():
            df_query_country = pd.read_feather(file_path_country)
        else:
    ...
    def get_query_region(povline, ppp_version):
        ...
        if Path(file_path_region).is_file():
            df_query_region = pd.read_feather(file_path_region)
        else:
            # Run the main function to get the data
...
def generate_consolidated_percentiles(df, wb_api: WB_API):
    ...
    path_file_percentiles = f"{CACHE_DIR}/world_bank_pip_percentiles_before_checks.feather"

    if Path(path_file_percentiles).is_file():
        log.info("Percentiles file already exists. No need to consolidate.")
        df_percentiles = pd.read_feather(path_file_percentiles)
        ...
        # Save to csv
        df_percentiles.to_feather(f"{CACHE_DIR}/world_bank_pip_percentiles_before_checks.feather")
    ...
    # Save to feather (for fast intermediate reads) and csv (final output)
    df_percentiles.to_feather(f"{CACHE_DIR}/world_bank_pip_percentiles.feather")
...
def generate_relative_poverty(wb_api: WB_API):
    ...
    def get_relative_data(df_row, pct, versions):
        ...
        if ~np.isnan(df_row["median"]):
            if Path(
                f"{CACHE_DIR}/pip_country_data/pip_country_{df_row['country_code']}_year_{df_row['year']}_povline_{int(round(df_row['median'] * pct))}_welfare_{df_row['welfare_type']}_rep_{df_row['reporting_level']}_fillgaps_{FILL_GAPS}_ppp_{PPP_VERSIONS[1]}.feather"
            ).is_file():
                return
            else:
                return pip_query_country(
    ...
    def get_relative_data_region(df_row, pct, versions):
        ...
        if ~np.isnan(df_row["median"]):
            if Path(
                f"{CACHE_DIR}/pip_region_data/pip_region_{df_row['country_code']}_year_{df_row['year']}_povline_{int(round(df_row['median'] * pct))}_ppp_{PPP_VERSIONS[1]}.feather"
            ).is_file():
                return
            else:
                return pip_query_region(
            ...
            for i in range(len(df)):
                if ~np.isnan(df["median"].iloc[i]) and df.iloc[i]["country_code"] not in OLD_REGIONS:
                    if country_or_region == "country":
                        ...
                        file_path = f"{CACHE_DIR}/pip_country_data/pip_country_{df.iloc[i]['country_code']}_year_{df.iloc[i]['year']}_povline_{int(round(df.iloc[i]['median'] * pct))}_welfare_{df.iloc[i]['welfare_type']}_rep_{df.iloc[i]['reporting_level']}_fillgaps_{FILL_GAPS}_ppp_{PPP_VERSIONS[1]}.feather"
                        if Path(file_path).is_file():
                            results = pd.read_feather(file_path)
                        else:
                            # Run the main function to get the data
                        ...
                        # Here I check if the file exists even after the original extraction. If it does, I read it. If not, I start the queries again.
                        file_path = f"{CACHE_DIR}/pip_region_data/pip_region_{df.iloc[i]['country_code']}_year_{df.iloc[i]['year']}_povline_{int(round(df.iloc[i]['median'] * pct))}_ppp_{PPP_VERSIONS[1]}.feather"
                        if Path(file_path).is_file():
                            results = pd.read_feather(file_path)
                        else:
                            # Run the main function to get the data
    ...
    # Save to csv
    df.to_feather(f"{CACHE_DIR}/pip_relative.feather")
...
def generate_key_indicators(wb_api: WB_API):
    ...
    # Save to csv
    df.to_feather(f"{CACHE_DIR}/pip_raw.feather")
...
def median_patch(df, country_or_region):
    ...
    # Read percentile file (feather for speed)
    df_percentiles = pd.read_feather(f"{CACHE_DIR}/world_bank_pip_percentiles.feather")
