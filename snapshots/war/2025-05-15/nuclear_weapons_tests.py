...
def run(upload: bool) -> None:
    ...
    date_published_raw = (
        soup.find("div", class_="field--name-field-review-date").find("div", class_="field--item").get_text(strip=True)
    )
    date_published = datetime.strptime(date_published_raw, "%B %Y").strftime("%Y-%m-%d")

    # Update publication date in metadata.
