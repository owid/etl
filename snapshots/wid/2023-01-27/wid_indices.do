...
foreach excl_option in $exclude_extrapolations {
	...
	*Get average and threshold income for pre tax and post tax (nat and dis) data
	wid, indicators(aptinc tptinc adiinc tdiinc acainc tcainc ahweal thweal) perc(p0p10 p10p20 p20p30 p30p40 p40p50 p50p60 p60p70 p70p80 p80p90 p90p100 p0p100 p99p100 p99.9p100 p99.99p100 p99.999p100) ages(992) pop(j) $exclude_option clear
	...
	*Gets shares and Gini for pre and post tax income
	wid, indicators(sptinc gptinc sdiinc gdiinc scainc gcainc shweal ghweal) perc(p0p10 p10p20 p20p30 p30p40 p40p50 p50p60 p60p70 p70p80 p80p90 p90p100 p0p100 p0p50 p50p90 p99p100 p99.9p100 p99.99p100 p99.999p100) ages(992) pop(j) $exclude_option clear
	...
	*Export csv
	export delimited using "wid_indices_992j_`excl_option_slug'.csv", replace
