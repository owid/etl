...
def run(upload: bool = True) -> None:
    ...
    for year in range(FIRST_YEAR, LAST_YEAR + 1):
        ...
        rows.append({"year": year, **{name: int(record[code]) for code, name in VARIABLES.items()}})

    df = pd.DataFrame(rows)

    assert len(df) == LAST_YEAR - FIRST_YEAR, "Unexpected number of years fetched."
    assert (df["foreign_born_population"] < df["total_population"]).all(), "Foreign-born exceeds total population."
