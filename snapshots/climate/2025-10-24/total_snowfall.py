"""Script to create a snapshot of the snowfall data from 1950 to present from the Copernicus Climate Change Service.
...
More information on how to access the data is here: https://cds.climate.copernicus.eu/datasets/reanalysis-era5-single-levels-monthly-means?tab=overview
...
def main(upload: bool) -> None:
    ...
    snap = Snapshot(f"climate/{SNAPSHOT_VERSION}/total_snowfall.zip")

    # Save data as a compressed temporary file.
    with tempfile.TemporaryDirectory() as temp_dir:
        output_file = Path(temp_dir) / "era5_monthly_snowfall.nc"
