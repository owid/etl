...
def main(upload: bool) -> None:
    ...
    snap = Snapshot(f"climate/{SNAPSHOT_VERSION}/surface_land_temperature.zip")
    # Save data as a compressed temporary file.
    with tempfile.TemporaryDirectory() as temp_dir:
        c = cdsapi.Client()
        output_file = Path(temp_dir) / "download.netcdf.zip"
        ...
        c.retrieve(
            "reanalysis-era5-land-monthly-means",
            {
                ...
                "year": [str(year) for year in range(1950, 2025)],
