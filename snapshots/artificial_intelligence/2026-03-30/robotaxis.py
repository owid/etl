...
def download_and_extract_csv_files(urls: List[str]) -> List[pd.DataFrame]:
    ...
    for url in urls:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Check for nested zip files and extract them too (exclude the main zip itself)
            nested_zips = [z for z in temp_path.rglob("*.zip") if z != zip_path]
            ...
            for csv_file in csv_files:
                ...
                if "TotalPMT" in df.columns:
                    ...
                    # Add carrier name based on TCPID
                    carrier_names = {
                        "PSG0038152": "Waymo",
                        "PSG0032513": "Waymo",  # Waymo's earlier permit ID (Dec 2022–Feb 2023)
                        "PSG0039080": "Cruise",
                    }
                    if "TCPID" in df.columns:
                        df["carrier_name"] = df["TCPID"].map(carrier_names).fillna("Unknown")
