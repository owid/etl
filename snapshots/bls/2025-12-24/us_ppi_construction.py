...
def main(upload: bool) -> None:
    ...
    # Fetch all monthly data including annual averages
    for year in range(start_year, end_year + 1, 10):
        chunk_start = year
        chunk_end = min(year + 9, end_year)

        log.info(f"Fetching data for {chunk_start}-{chunk_end}")
        data = json.dumps(
    ...
    # Process data - keep both monthly and annual data
    flattened = []
    for series in all_series_data:
        ...
        for item in series["data"]:
            ...
            # Add month number for monthly data
