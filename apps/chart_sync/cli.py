...
def cli(
    ...
    **Example 4:** Ignore conflicts when syncing between staging servers (useful if conflicts with master have already been dealt with in a subbranch server)
    ...
    source_api: AdminAPI | None = AdminAPI(source_env) if not dry_run else None
    target_api: AdminAPI | None = AdminAPI(target_env) if not dry_run else None
    ...
    with Session(source_engine) as source_session:
        with Session(target_engine) as target_session:
            ...
            # Sync narrative charts for the synced parent charts (requires APIs, so skip in dry_run)
            if not dry_run:
                narrative_charts_synced = _sync_narrative_charts(
                    synced_chart_ids, source_session, target_session, source_api, target_api
                )
