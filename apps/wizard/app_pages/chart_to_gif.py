...
if "images_exist" not in st.session_state:
    st.session_state.images_exist = False
...
st.session_state.images_exist = (
    len([image for image in st.session_state.images_folder.iterdir() if image.suffix == ".png"]) > 0
)
...
if st.session_state.get("show_gif_settings"):
    ...
    if st.session_state.gif_file.exists():
        ...
        st.image(str(st.session_state.gif_file), use_container_width=True)
