...
def st_show_indicator(indicator, indicator_charts):
    ...
    with st.container(border=False):
        ...
        with st_metadata_right:
            ...
            # Charts
            df_charts = get_table_charts(indicator_charts, var.id)
            show_table_charts(df_charts)

            # Explorers
            df_explorers = get_table_explorers(indicator_explorers, var.id)
...
if DATASET_ID is not None:
    ...
    # 2/ Get charts
    df_charts = get_table_charts(indicator_charts)
    df_explorers = get_table_explorers(indicator_explorers)
