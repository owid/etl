...
from apps.chart_critic.critic import format_views, issue_params
...
def _finding(result: dict[str, Any], issue: dict[str, Any], png: bytes | None, params: str) -> str:
    ...
    parts = [
        ...
        f'<span class="tag lo">{format_views(result["views"])}</span>' if result["views"] else "",
        "</div>",
...
def write(results: list[dict[str, Any]], path: Path, model: str) -> None:
    ...
    # Charts nobody reviewed. They have no issues, so without their own section they would sit in
    # the "nothing to report" chips and a run where every single chart failed would be headlined
    # "No issues found" — the report is written before the command exits 2, so it outlives the
    # exit code as the artifact people actually read.
    failed = [r for r in results if r["status"] not in ("ok", "gone")]
    ...
    chips = "\n".join(
        f'<span class="chip"><b>{html.escape(r["slug"])}</b><i>{format_views(r["views"])}</i></span>'
        if r["views"]
    ...
    if flagged:
        headline = (
            f'<div class="note"><b>{n_issues} issue(s) across {len(flagged)} of {len(results)} charts.</b> '
        ...
        headline = (
            f'<div class="note"><b>No issues found in the {len(clean)} charts that were reviewed — '
            f"but {len(failed)} could not be reviewed at all.</b> This run is incomplete, so it is "
            "not evidence that those charts are fine.</div>"
        )
    else:
        headline = f'<div class="note ok"><b>No issues found</b> in {len(clean)} charts.</div>'

    failed_section = (
        '<h2>Not reviewed</h2>\n<div class="chips">\n'
        + "\n".join(
            f'<span class="chip gone"><b>{html.escape(r["slug"])}</b><i>{html.escape(r["status"])}</i></span>'
            for r in failed
        )
        + "\n</div>"
        if failed
