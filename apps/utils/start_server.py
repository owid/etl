"""This script creates a git branch, makes an empty commit, and creates a draft pull request.
...
* The base branch will be the current branch, where your ETL repos currently is.
...
def cli(new_branch_name: str) -> None:
    if not GITHUB_TOKEN:
    ...
    if new_branch_name in [branch.name for branch in repo.branches]:
        log.error(
            f"Branch {new_branch_name} already exists. " "Choose a different name for the new branch to be created."
        )
        return
