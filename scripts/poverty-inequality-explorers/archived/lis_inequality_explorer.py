...
for tab in range(len(tables)):
    for eq in range(len(equivalence_scales)):
        ...
        # Headcount ratio (rel)
        df_graphers.loc[j, "title"] = "Share of people in relative poverty (after tax vs. before tax)"
        df_graphers.loc[j, "ySlugs"] = (
...
save("inequality-lis", tables, df_header, df_graphers, df_tables)  # ty: ignore
