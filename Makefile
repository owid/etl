...
help:
	...
	@echo '  make etl-mcp-setup 	Set up MCP servers for Claude from .mcp.json'
...
etl-mcp-setup: .venv
	@echo '==> Setting up ETL MCP server for Claude'
	claude mcp remove etl-owid -s project 2>/dev/null || true; \
	claude mcp add-json etl-owid '{"type": "stdio", "command": ".venv/bin/fastmcp", "args": ["run", "etl_mcp/server.py"], "env": {}}'; \
