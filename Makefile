...
watch-all:
	uv run watchmedo shell-command -c 'clear; make unittest; for lib in $(LIBS); do (cd $$lib && make unittest); done' --recursive --drop .
...
watch: .venv
	uv run watchmedo shell-command -c 'clear; make check-formatting lint check-typing coverage' --recursive --drop .
...
coverage: .venv
	...
	uv run pytest --cov=etl --cov-report=term-missing tests

etl: .venv
	@echo '==> Running etl on garden'
	uv run etl run garden
...
publish: etl reindex
	...
	uv run etl d publish --private

reindex: .venv
	@echo '==> Creating a catalog index'
	uv run etl d reindex
...
version-tracker: .venv
	...
	uv run etl d version-tracker

api: .venv
	@echo '==> Starting ETL API on http://localhost:8081/api/v1/indicators'
	uv run uvicorn api.main:app --reload --port 8081 --host 0.0.0.0

fasttrack: .venv
	@echo '==> Starting Fast-track on http://localhost:8082/'
	uv run fasttrack --skip-auto-open --port 8082
