...
- **`description_key` is a markdown string once it reaches the grapher channel** — a YAML list is authoring sugar that survives garden (so per-item Jinja can render) and is joined into markdown in `update_variable_metadata`. Steps reading `data://grapher/...` — mdim and explorer `export://` steps especially — must pass the value through, never iterate it or rebuild it as a list: `list("- a\n- b")` yields one bullet *per character*, and every write path used to silently rejoin that into valid-looking markdown. That shipped ~2,900 one-character WYSK bullets to readers (#6647); the guard in `validate_description_key_list` now raises instead.
