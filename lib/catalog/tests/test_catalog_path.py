...
| Class                    | Description                                                              |
|--------------------------|--------------------------------------------------------------------------|
| TestFromStr              | Parsing catalog paths from strings (full paths, dataset-only, URIs)     |
| TestFromStrValidation    | Validation errors during parsing (invalid channel, version, characters) |
| TestProperties           | Property accessors (dataset_path, table_path, step_uri, short_name)     |
| TestWithMethods          | Immutable builder methods (with_version, with_channel, with_table, etc) |
| TestStringRepresentation | String output (__str__, __repr__, roundtrip parsing)                    |
| TestHashingAndEquality   | Hashing and equality (dict keys, sets, string comparison)               |
| TestParentProperty       | Parent path navigation (removing variable/table components)             |
| TestTruedivOperator      | Path joining with / operator (appending table and variable)             |
| TestFspathProtocol       | os.fspath() and pathlib.Path compatibility                              |
| TestAdditionalCoverage   | Edge cases (all channels, direct construction, immutability)            |
"""
...
class TestAdditionalCoverage:
    ...
    def test_with_namespace(self) -> None:
        """with_namespace creates new path with different namespace."""
        p = CatalogPath.from_str("grapher/who/2024/gho/table")
        p2 = p.with_namespace("un")
        assert p2.namespace == "un"
        assert p.namespace == "who"

    def test_with_dataset(self) -> None:
        """with_dataset creates new path with different dataset."""
        p = CatalogPath.from_str("grapher/who/2024/gho/table")
        p2 = p.with_dataset("wdi")
        assert p2.dataset == "wdi"
        assert p.dataset == "gho"

    def test_from_uri_alias(self) -> None:
        """from_uri is an alias for from_str."""
        p1 = CatalogPath.from_str("data://grapher/who/2024/gho")
        p2 = CatalogPath.from_uri("data://grapher/who/2024/gho")
        assert p1 == p2

    def test_snapshot_private_uri(self) -> None:
        """Parse snapshot-private:// URI."""
        p = CatalogPath.from_str("snapshot-private://who/2024-01-01/data")
        assert p.channel == "snapshot"
        assert p.namespace == "who"

    def test_snapshot_with_variable_raises(self) -> None:
        """Snapshot paths cannot have variables."""
        with pytest.raises(ValueError, match="Snapshot paths cannot have indicators"):
            CatalogPath.from_str("snapshot://who/2024-01-01/data#var")

    def test_direct_construction(self) -> None:
        """Can construct CatalogPath directly."""
        p = CatalogPath(
            channel="garden",
            namespace="un",
            version="2024-01-01",
        ...
        assert str(p) == "garden/un/2024-01-01/population/estimates#total"

    def test_direct_construction_validation(self) -> None:
    ...
    def test_variable_without_table_raises(self) -> None:
        """Cannot have variable without table."""
        with pytest.raises(ValueError, match="Cannot have variable without table"):
            CatalogPath(
                channel="garden",
                namespace="un",
                version="2024",
