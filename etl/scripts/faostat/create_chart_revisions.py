    ...
    with db.get_connection() as db_conn:
        try:
            ...
            dataset_id_old = db.get_dataset_id(
                db_conn=db_conn, dataset_name=dataset_old.metadata.title, version=dataset_old.metadata.version  # type: ignore[reportArgumentType]
            )
        ...
        try:
            dataset_id_new = db.get_dataset_id(
                db_conn=db_conn, dataset_name=dataset_new.metadata.title, version=dataset_new.metadata.version  # type: ignore[reportArgumentType]
            )
        except AssertionError:
    ...
    for dataset_short_name in dataset_short_names:
        ...
        if execute_revisions and len(grapher_variable_ids_mapping) > 0:
            ...
            raise NotImplementedError(
                "This script made use of old chart revisions workflow, which is no longer supported."
            )
            # try:
