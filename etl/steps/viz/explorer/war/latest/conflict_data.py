...
chart — no view rewriting, no `create_explorer` call:

  1. Load the upstream `conflict_data_source` chart from the local export.
...
def run() -> None:
    ...
    cs = paths.load_chartset("conflict_data_source", channel="explorer")
