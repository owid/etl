...
def update_titles(dry_run: bool):
    ...
    for indicator_code in df_vars.index:
        ...
        if indicator_code in variables:
            ...
            # NOTE: compare stripped. The World Bank's legacy API returns some indicator names with a
            # trailing space, and the write below strips them -- so an unstripped comparison reported
            # "title updates" that write back a byte-identical YAML, every single cycle (3 such
            # indicators as of 2026-07-27: ic_frm_bkwc_zs, sg_tim_uwrk_ma, sl_emp_smgt_fe_zs).
            if old_title.strip() != indicator_name.strip():
