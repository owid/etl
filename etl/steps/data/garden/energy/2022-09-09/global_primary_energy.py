...
from shared import CURRENT_DIR, gather_sources_from_tables, combine_two_overlapping_dataframes
...
# This factor will be used for hydropower, nuclear, other renewables, solar and wind (for which there is data until 1960).
...
def prepare_bp_data(tb_bp: catalog.Table) -> pd.DataFrame:
    ...
    # More specifically, BP assumes (for Biofuels, Coal, Gas and Oil) an efficiency factor that grows from 36% (until year 2000) to 40.6% (in 2021), to better reflect changes in efficiency over time.
    # In the case of biomass used in electricity (included in 'Other renewables'), BP assumes a constant factor of 32% for all years.
    # For more details: https://www.bp.com/content/dam/bp/business-sites/en/global/corporate/pdfs/energy-economics/statistical-review/bp-stats-review-2022-methodology.pdf
    bp_columns = {
        'country': "country",
        "year": "year",
    ...
    # For completeness, create columns of substituted energy for fossil sources (even if they would coincide with direct energy).
...
def prepare_smil_data(tb_smil: catalog.Table) -> pd.DataFrame:
    ...
    # (hydropower, nuclear, other renewables, solar and wind) by a factor of 36% (called EFFICIENCY_FACTOR, defined above).
    # This is the efficiency factor of a typical thermal plant assumed by BP between 1965 and 2000, and we assume this factor also applies for the period 1800 to 1965.
    # For biomass power (included in other renewables), BP assumed a constant factor of 32%.
    # However, since we cannot separate biomass from the rest of sources in 'other renewables', we use the same 36% factor as all other non-fossil sources.
...
def combine_bp_and_smil_data(df_bp: pd.DataFrame, df_smil: pd.DataFrame) -> pd.DataFrame:
    ...
    combined = combine_two_overlapping_dataframes(df1=df_bp, df2=df_smil, index_columns=["country", "year"]).sort_values(["year"])
