...
def combine_beis_and_electricity_mix_data(df_beis: pd.DataFrame, df_elec: pd.DataFrame) -> pd.DataFrame:
    ...
    solar_or_wind_first_year = df_elec[df_elec["wind_generation"].notnull() | df_elec["solar_generation"].notnull()]["year"].min()
    ...
    # Similarly, given that in the BEIS dataset there is no data about other renewable sources (apart from hydro, solar and wind),
    # we can assume that the contribution from other renewables is zero.
    ...
    df_beis.loc[df_beis["year"] >= solar_or_wind_first_year, ["solar_generation", "wind_generation", "other_renewables_generation"]] = np.nan
    ...
    # We multiply the raw energy by this efficiency to have an estimate of the electricity generated by each fossil fuel.
    # This only affects data prior to the beginning of the electricity mix's data (which is 1965 for renewables and nuclear,
    # and 1985 for the rest).
    ...
    df_combined = dataframes.combine_two_overlapping_dataframes(df1=df_elec, df2=df_beis, index_columns=["country", "year"])
