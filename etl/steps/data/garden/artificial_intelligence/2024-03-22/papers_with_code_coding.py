...
def run(dest_dir: str) -> None:
    # Load inputs.
    # Load meadow dataset.
    ds_meadow = paths.load_dataset("papers_with_code_coding")

    # Read table from meadow dataset.
    tb = ds_meadow["papers_with_code_coding"].reset_index()
    ...
    grouped_interview = group_sort_filter(tb, "performance_code_any_interview")
    grouped_competition = group_sort_filter(tb, "performance_code_any_competition")

    tb_combined = pr.merge(grouped_interview, grouped_competition, on=["days_since", "name"], how="outer")

    tb_combined = tb_combined.set_index(["days_since", "name"], verify_integrity=True).sort_index()
