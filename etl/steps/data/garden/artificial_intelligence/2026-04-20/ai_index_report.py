...
def run() -> None:
    ...
    # ── AI Adoption ─────────────────────────────────────────────────────────────
    tb_adoption = ds_meadow.read("ai_adoption")
    ...
    tb_adoption_old = ds_old_adoption.read("ai_adoption")
    ...
    # ── AI Bills ────────────────────────────────────────────────────────────────
    tb_bills = ds_meadow.read("ai_bills")
    tb_bills = paths.regions.harmonize_names(tb_bills, warn_on_unused_countries=False)
    tb_bills = tb_bills.format(["country", "year"], short_name="ai_bills")
    tables.append(tb_bills)

    # ── AI Incidents ─────────────────────────────────────────────────────────────
    tb_incidents = ds_meadow.read("ai_incidents")
    tb_incidents = tb_incidents.format(["year", "country"], short_name="ai_incidents")
    tables.append(tb_incidents)

    # ── AI Conferences ───────────────────────────────────────────────────────────
    tb_conferences = ds_meadow.read("ai_conferences")
    ...
    tb_conferences = tb_conferences.format(["conference", "year"], short_name="ai_conferences")
    tables.append(tb_conferences)

    # ── AI Jobs ──────────────────────────────────────────────────────────────────
    tb_jobs = ds_meadow.read("ai_jobs")
    ...
    tb_jobs = tb_jobs.format(["country", "year"], short_name="ai_jobs")
    tables.append(tb_jobs)

    # ── Investment: Generative AI ────────────────────────────────────────────────
    tb_gen = ds_meadow.read("ai_investment_generative")
    ...
    tb_gen = tb_gen.format(["year", "country"], short_name="ai_investment_generative")
    tables.append(tb_gen)

    # ── Investment: Newly funded companies ──────────────────────────────────────
    tb_companies = ds_meadow.read("ai_investment_companies")
    tb_companies = tb_companies.rename(columns={"number_of_companies": "companies"})
    tb_companies = tb_companies.format(["year", "country"], short_name="ai_new_companies")
    tables.append(tb_companies)

    # ── Investment: Private (World + regions) ────────────────────────────────────
    tb_region = ds_meadow.read("ai_investment_by_region")
    ...
    tb_region = tb_region.format(["year", "country"], short_name="ai_private_investment")
    tables.append(tb_region)

    # ── Investment: Corporate deals by event type ────────────────────────────────
    tb_corp = ds_meadow.read("ai_investment_corporate")
    ...
    tb_corp = tb_corp.format(["year", "investment_type"], short_name="ai_corporate_investment")
    tables.append(tb_corp)

    # ── Industrial Robots ─────────────────────────────────────────────────────────
    tb_ind = ds_meadow.read("ai_robots_industrial")
    ...
    tb_ind = tb_ind.format(["country", "year"], short_name="industrial_robots")
    tables.append(tb_ind)

    # ── Professional Robots ───────────────────────────────────────────────────────
    tb_prof = ds_meadow.read("ai_robots_professional")
    ...
    tb_prof = tb_prof.format(["year", "application_area"], short_name="professional_robots")
