...
def run() -> None:
    ...
    # Load inputs.
    #
    # Load new meadow dataset (2023-2025 + 2030 projection).
    ds_meadow = paths.load_dataset("energy_ai", version="2026-04-27")
    tb_new = ds_meadow.read("energy_ai")

    # Load old meadow dataset (has 2020 data not present in the new file).
    ds_meadow_old = paths.load_dataset("energy_ai", version="2025-11-07")
    tb_old = ds_meadow_old.read("energy_ai")

    # Keep only 2020 data from the old dataset (years covered by the new file start from 2023).
    tb_old = tb_old[tb_old["year"] < 2023].reset_index(drop=True)

    # Combine: old historical data + new data.
    tb = pr.concat([tb_old, tb_new], ignore_index=True)
