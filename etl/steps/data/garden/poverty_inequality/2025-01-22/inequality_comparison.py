...
def match_ref_years(df: Table, series: list, reference_years: dict, only_all_series: bool) -> pd.DataFrame:
    """
    ...
    for y in reference_years:
        ...
        if df_match.empty:
            ...
            if len(reference_years_list) == 2:
                ...
                df_match["pipreportinglevelcat"] = df_match.apply(
                ...
                # Categorise the pipwelfare match
                df_match["pipwelfarecat"] = df_match.apply(
    ...
    # add regions
    df_match = pd.merge(df_match, df_regions, how="left")

    for y in reference_years_list:
        df_pop_year = df_pop[df_pop["year"] == y]
        df_pop_year = df_pop_year.drop(columns="year")

        df_match = pd.merge(df_match, df_pop_year, how="left")
...
for ref_yrs in ref_yrs_list:
    ...
    owid_data = pd.wide_to_long(
        output_main, ["value", "year", "population"], i=["country", "series_code"], j="ref_year", sep="_"
    ).reset_index()
    ...
    output_main_non_varying = output_main.drop_duplicates(subset=["indicator_name", "country"])[
    ...
    output_matched.to_csv(
        f"data/selected_observations/{ref_yrs[0]}_{ref_yrs[1]}_by_country_series_code_added_wid_extrapolations.csv",
        index=False,
    )
    ...
    # Save to csv
    output_all_obs.to_csv(
        f"data/selected_observations/{ref_yrs[0]}_{ref_yrs[1]}_by_indicator_owid_upload.csv", index=False
    )
