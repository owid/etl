...
def run_sanity_checks(tb: Table) -> None:
    error = "Percentages do not add up to 100% for some of the surveyed dates (within 2.5%)."
    assert (abs(tb.drop(columns=["base", "base_unweighted"]).sum(axis=1) - 100) <= 2.5).all(), error
