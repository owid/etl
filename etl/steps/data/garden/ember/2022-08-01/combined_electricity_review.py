...
DATASET_SHORT_NAME = "combined_electricity_review"
METADATA_PATH = CURRENT_DIR / f"{DATASET_SHORT_NAME}.meta.yml"
# Details for datasets to import.
GLOBAL_DATASET_PATH = DATA_DIR / f"garden/ember/2022-08-01/global_electricity_review"
EUROPEAN_DATASET_PATH = DATA_DIR / f"garden/ember/2022-08-01/european_electricity_review"
...
def combine_global_electricity_review_data(ds_global: catalog.Dataset) -> catalog.Table:
    ...
    error = "Tables in global electricity review dataset have changed"
    ...
    for category in category_renaming:
        ...
        table = table.rename(columns={column: catalog.utils.underscore(category_renaming[category] + column)
                                      for column in table.columns if column not in index_columns})
    ...
    combined_global = combined_global.rename(columns={
    ...
    assert all(combined_global["emissions__total_generation__twh"] ==
               combined_global["generation__total_generation__twh"]), error
...
def combine_european_electricity_review_data(ds_european: catalog.Dataset) -> catalog.Table:
    index_columns = ["country", "year"]
    ...
    check = pd.merge(ds_european["country_overview"].reset_index()[columns], generation.reset_index()[columns],
                     on=index_columns)
    # Assert that the percentage change is smaller than 1%
    error = "Total generation does not agree with the on in country_overview."
    assert all((abs(check["total_generation__twh_x"] - check["total_generation__twh_y"]) /
                check["total_generation__twh_x"]) < 0.01), error
    ...
    country_overview = country_overview.rename(columns={
...
def combine_global_and_european_electricity_review_data(
        combined_global: catalog.Table, combined_european: catalog.Table) -> catalog.Table:
    ...
    index_columns = ["country", "year"]
    data_columns = sorted([col for col in (set(combined_global.columns) | set(combined_european.columns))
    ...
    # Import metadata from metadata yaml file.
    combined.update_metadata_from_yaml(METADATA_PATH, "combined_electricity_review")
...
def run(dest_dir: str) -> None:
    ...
    # Process data.
    #
    # Combine all tables of the global electricity review into one.
