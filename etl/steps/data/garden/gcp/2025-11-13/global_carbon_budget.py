...
def sanity_checks_on_input_data(
    ...
    # In the original data, International Aviation and International Shipping were included in the national data file, as countries.
    # But they should be considered as another kind of global emission.
    # In fact, they should coincide for production and consumption emissions.
    global_bunkers_emissions = (
        tb_production[tb_production["country"].isin(["International Aviation", "International Shipping"])][
            ["year", "country", "production_emissions"]
        ]
        .dropna()
        .groupby("year", as_index=False)
        .agg({"production_emissions": "sum"})
        .reset_index(drop=True)
        .rename(columns={"production_emissions": "global_bunker_emissions"}, errors="raise")
    )
    error = "International Aviation and International Shipping emissions in the national data file may have changed."
    assert (global_bunkers_emissions["global_bunker_emissions"] != 0).all(), error

    # Check that we get exactly the same array of emissions from the consumption emissions table
    # (on years where there is data for international aviation and shipping emissions in both datasets).
    comparison = pr.merge(
        global_bunkers_emissions,
        tb_consumption[tb_consumption["country"].isin(["International Aviation", "International Shipping"])][
            ["year", "country", "consumption_emissions"]
        ]
        .dropna()
        .groupby("year", as_index=False)
        .agg({"consumption_emissions": "sum"})
        .reset_index(drop=True)
    ...
    error = "International Aviation and International Shipping emissions were expected to coincide in production and consumption emissions tables."
    ...
    # Check that, for the World, production emissions coincide with consumption emissions (on common years).
    ...
    # Check that International Aviation and International Shipping from the national emissions file coincide with those from the Fossil CO2 file.
    global_aviation_and_shipping = (
        tb_co2[tb_co2["country"].isin(["International Aviation", "International Shipping"])]
        .dropna()
        .pivot(index="year", columns="country", values="emissions_total")
        .reset_index()
    )
    global_aviation_and_shipping["global_aviation_and_shipping"] = (
        global_aviation_and_shipping["International Aviation"] + global_aviation_and_shipping["International Shipping"]
    )
    comparison = (
        global_bunkers_emissions.merge(
            global_aviation_and_shipping[["year", "global_aviation_and_shipping"]],
            how="outer",
            on="year",
        )
        .sort_values("year")
    ...
    # Keep only rows where both time series are informed.
    comparison = comparison.dropna(
        subset=["global_bunker_emissions", "global_aviation_and_shipping"], how="any"
    ).reset_index(drop=True)
    error = "International Aviation and International Shipping emissions from the national emissions file should coincide (within 0.0001%) with the ones from the Fossil CO2 file."
    assert (
        100
    ...
    # Now check that all other emissions in tb_production (emissions from the national excel file) coincide with emissions in tb_co2 (from the Fossil CO2 emissions csv file).
    ...
    comparison = pr.merge(
        ...
        tb_production.astype({"country": str}).replace({"World": "Global"}),
        on=["country", "year"],
        how="inner",
    ...
    # NOTE: It seems that total emissions may have been rounded to zero decimals, which is why in the following assertion I also round production emissions.
    error = "Production emissions from national file were expected to coincide with the Fossil CO2 emissions dataset."
    assert (
        (
            100
            * abs(comparison["production_emissions"].round(0) - comparison["emissions_total"])
...
def sanity_checks_on_output_data(tb_combined: Table) -> None:
    ...
    assert tb_combined[
        (tb_combined["country"] == "World") & (abs(tb_combined["emissions_total_as_share_of_global"] - 100) > 0.00001)
    ].empty, error
    ...
    error = "All share of global emissions should be smaller than 100%."
    share_variables = [
        col
        for col in tb_combined.columns
        if "share" in col
    ...
    assert (tb_combined[share_variables].fillna(0) <= 100.001).all().all(), error
    # NOTE: In previous versions, "emissions_from_land_use_change_as_share_of_global" was >101%, e.g. from Upper-middle-income countries in 1982, 1984 and 1986. This is, in principle, possible (since land use change emissions can be negative).
    assert (tb_combined["emissions_from_land_use_change_as_share_of_global"].fillna(0) <= 100.00001).all().all(), error
    ...
    # a percentage change that is smaller than a certain amount.
    error = (
    ...
    assert (tb_combined[tb_combined["country"] == "World"][share_variables].fillna(100) > 99.9999).all().all(), error

    error = "Traded emissions for the World should be close to zero."
...
def extract_global_emissions(tb_co2: Table, tb_historical: Table, ds_population: Dataset) -> Table:
    ...
    names in tb_co2 (so that "International Aviation" and "International Shipping" are still listed as countries).
    ...
    """
    # "International Aviation" and "International Shipping" are now included as separate countries.
    # Combine their emissions into one variable.
    global_aviation = (
        tb_co2[tb_co2["country"] == "International Aviation"].set_index(["year"]).drop(columns=["country"])
    )
    global_shipping = (
        tb_co2[tb_co2["country"] == "International Shipping"].set_index(["year"]).drop(columns=["country"])
    )
    global_transport = global_aviation + global_shipping

    # Check that total emissions for international aviation coincide with oil emissions.
    # NOTE: International shipping does include emissions from gas, coal and oil.
    error = "Total emissions from international aviation do not coincide with oil emissions."
    assert all((global_aviation["emissions_from_oil"] - global_aviation["emissions_total"]).dropna() == 0), error

    # Keep only total emissions from international transport.
    global_transport = (
        global_transport[["emissions_total"]]
        .rename(columns={"emissions_total": "global_emissions_from_international_transport"}, errors="raise")
        .dropna()
        .reset_index()
    )
...
def combine_data_and_add_variables(
    ...
    # Temporarily add global emissions and global cumulative emissions columns to main table, to be able to calculate
    # indicators in terms of global emissions.
    ...
    tb_co2_with_regions["global_consumption_emissions"] = tb_co2_with_regions["global_emissions_total"].copy()
    ...
    # Remove temporary columns of global emissions.
