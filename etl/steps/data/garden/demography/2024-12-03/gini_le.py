...
def run(dest_dir: str) -> None:
    ...
    paths.log.info("gini_le: keep relevant dimensions (type='period', sex in ['male', 'female'])")
    ...
    # Estimates (this can take a half minute or so, depending on computation power)
    tb = tb.groupby(["country", "year", "sex"], as_index=False, observed=False).apply(gini_from_mx)
    ...
    tb = tb.format(["country", "year", "sex"], short_name=paths.short_name)
