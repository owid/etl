"""Long-run population OMM — Federico-Tena variant (assesses the swap proposed in
...
# Gapminder ##########
######################
def format_ft(tb: Table) -> Table:
    """Format the Federico-Tena V2 population table (already harmonized in its garden step)."""
    # F-T garden output is (country, year, population, continent, historical_name) — keep core cols
    tb = tb.loc[:, COLUMS_RELEVANT_POP].copy()
    tb["source"] = "ft"
    return tb


def make_ft_interp(tb_ft: Table, tb_un: Table) -> Table:
    """Linear-interpolate the 1939-1949 gap (F-T(1938) → UN WPP(1950)) per country.

    For each country present in both F-T (at 1938) and UN WPP (at 1950) we generate yearly rows
    1939-1949 with `population = F-T(1938) + (WPP(1950) - F-T(1938)) * (y - 1938) / 12`.
    """
    ft_1938 = (
        tb_ft.loc[tb_ft["year"] == YEAR_END_FT, ["country", "population"]]
        .set_index("country")["population"]
        .astype(float)
    )
    wpp_1950 = (
        tb_un.loc[tb_un["year"] == YEAR_START_WPP, ["country", "population"]]
        .set_index("country")["population"]
        .astype(float)
    )
    common = ft_1938.index.intersection(wpp_1950.index)
    rows = []
    span = YEAR_START_WPP - YEAR_END_FT  # 12
    for c in common:
        v0, v1 = ft_1938[c], wpp_1950[c]
        for yr in range(GAP_LO, GAP_HI + 1):
            rows.append((c, yr, int(round(v0 + (v1 - v0) * (yr - YEAR_END_FT) / span))))
    if not rows:
        return Table(pd.DataFrame(columns=COLUMS_RELEVANT_POP + ["source"]))
    out = Table(pd.DataFrame(rows, columns=COLUMS_RELEVANT_POP))
    out["source"] = "ft_interp"
    # carry F-T's population metadata so origins propagate to the interpolated rows
    out["population"].metadata = tb_ft["population"].metadata.copy()
    return out
...
def select_source(tb: Table) -> Table:
    ...
    Prioritisation:
    - For 1800-2100: WPP > F-T (incl. F-T interpolation) > HYDE.
    - Prior to 1800: HYDE.

    If a country has F-T data, we drop HYDE for that country in 1800-1949 (so the F-T series
    runs unbroken, with the interpolation block bridging 1939-1949).
    """
    paths.log.info("selecting source...")
    tb = tb.loc[tb["population"] > 0]

    # If a country has UN WPP data, drop all non-UN data >=1950
    ...
    # If a country has F-T data, drop non-F-T data between 1800 and 1949 (keep both "ft" and
    # "ft_interp" — they cover 1800-1938 and 1939-1949 respectively for the same country).
    ft_sources = {"ft", "ft_interp"}
    has_ft_data = set(tb.loc[tb["source"] == "ft", "country"])
    tb = tb.loc[
        ~(
            tb["country"].isin(has_ft_data)
            & (tb["year"] >= YEAR_START_FT)
            & (tb["year"] < YEAR_START_WPP)
            & ~tb["source"].isin(ft_sources)
        )
    ]
...
def add_regions(tb: Table, ds_regions: Dataset, ds_income_groups: Dataset) -> Table:
    ...
    tb_agg.loc[(tb_agg["year"] < YEAR_START_FT) & tb_agg["source"].isna(), "source"] = SOURCES_NAMES["hyde"]
    # NA rows in the 1800-1949 block were filled by interpolation of F-T values — label them as F-T
    tb_agg.loc[
        (tb_agg["year"] >= YEAR_START_FT) & (tb_agg["year"] < YEAR_START_WPP) & tb_agg["source"].isna(), "source"
    ] = SOURCES_NAMES["ft"]
...
def generate_auxiliary_table(tb: Table) -> Table:
    ...
    origins = [
        Origin(
            ...
            title="Population",
            # NEW-OMM variant (Federico-Tena replaces Gapminder for 1800-1938; gap interp 1939-1949).
            # The "(NEW)" tag is intentional — it lets chart footers visually distinguish prod
            # (which still says "...sources (2024)") from staging at a glance during the A/B.
            attribution="Population based on various sources (NEW)",
            attribution_short="Population (NEW)",
