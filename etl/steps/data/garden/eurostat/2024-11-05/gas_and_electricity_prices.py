...
FLAGS = {
    ...
    # I couldn't find the meaning of the following flag.
...
def run(dest_dir: str) -> None:
    ...
    error = "Unexpected currencies."
    assert set(tb["currency"].dropna()) == set(CURRENCIES), error
    error = "Unexpected price levels."
    assert set(tb["tax"].dropna()) == set(PRICE_LEVELS), error
    error = "Unexpected energy units."
    ...
    # Map flags to their names.
    # TODO: Instead, do a loop for all fields, and create a nested dictionary with all mappings.
    tb.loc[tb["flags"].notnull(), "flags"] = map_series(
        tb[tb["flags"].notnull()]["flags"],
        mapping=FLAGS,
        warn_on_missing_mappings=True,
        warn_on_unused_mappings=True,
        show_full_warning=True,
