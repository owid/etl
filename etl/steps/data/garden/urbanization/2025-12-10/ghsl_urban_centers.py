...
def run() -> None:
    ...
    ####################################################################################################################
    # City size distribution: restrict to regions only.
    tb_city_sizes = tb_city_sizes[tb_city_sizes["country"].isin(REGIONS)].copy()
    ...
    # - tb_city_sizes: regional aggregates only with city size distributions and shares
