...
def run(dest_dir: str) -> None:
    ...
    for column in tb_pivoted.columns:
        metadata = tb_pivoted[column].metadata
        metadata.display = {}
        long_definition = tb["long_description"].loc[tb["indicator_label_en"] == column].iloc[0]
        metadata.description_from_producer = long_definition
        ...
        if "%" in column.lower():
            update_metadata(metadata, display_decimals=1, unit="%", short_unit="%")
        elif "(days)" in column.lower():
            update_metadata(metadata, display_decimals=1, unit="days", short_unit="")
        elif "index" in column.lower():
            update_metadata(metadata, display_decimals=1, unit="index", short_unit="")
        elif "(current us$)" in column.lower():
            update_metadata(metadata, display_decimals=1, unit="current US$", short_unit="current $")
        elif "(number)" in column.lower():
            update_metadata(metadata, display_decimals=0, unit="number", short_unit="")

        else:
            # Default metadata update when no other conditions are met.
            update_metadata(metadata, 0, " ", " ")
...
def update_metadata(meta: Dict[str, Any], display_decimals: int, unit: str, short_unit: str) -> None:
    """
    Update metadata attributes of a specified column in the given table.

    Args:
    meta (Dict[str, Any]): Metadata object of the column to update.
