...
def run() -> None:
    ...
    tb_ratings = ds_meadow.read("fh_ratings")
    tb_scores = ds_meadow.read("fh_scores")
    ...
    # tb_regions.loc[tb_regions["year"] < 2005, "num_regime"] = pd.NA
    tb_regions.loc[tb_regions["year"] < 2005, "num_electdem"] = pd.NA
    tb_regions.loc[tb_regions["year"] < 2005, "pop_electdem"] = pd.NA
    tb_regions.loc[(tb_regions["year"] == 1981) | (tb_regions["year"] < 1972), "pop_regime"] = pd.NA
    # tb_regions.loc[(tb_regions["year"] == 1981) & (tb_regions["year"] < 1972), "pop_electdem"] = pd.NA
...
def add_electdem(tb: Table) -> Table:
    ...
    mask = (tb["electprocess"] >= 7) & (tb["polrights_score"] >= 20) & (tb["civlibs_score"] >= 30)
    tb.loc[mask, "electdem"] = 1
    tb.loc[
        ~mask & (tb[["electprocess", "polrights_score", "civlibs_score"]].notna().all(axis=1)),
        "electdem",
    ] = 0

    tb["electdem"] = tb["electdem"].copy_metadata(tb["electprocess"])
    return tb
