...
def add_metadata(tb: Table, metadata_tb: Table) -> None:
    ...
    for column in tqdm(tb.columns, desc="Processing metadata for indicators"):
        if column not in custom_cols and column not in ["country", "year"]:
            ...
            description = metadata_tb.loc[metadata_tb["indicator_code"] == column, "source_note"].iloc[0]
            source = metadata_tb.loc[metadata_tb["indicator_code"] == column, "source"].iloc[0]
