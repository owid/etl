...
def run() -> None:
    ...
    #
    # Load meadow dataset.
    ds_meadow = paths.load_dataset("atus_time_spent_with")
    ...
    tb_agg_full = aggregate_over_age(tb_agg.copy(), start_year=2010, end_year=END_YEAR)

    # tables for gender, full:
    tb_agg_female = aggregate_over_age(tb_agg, start_year=2010, end_year=END_YEAR, gender="female")
    tb_agg_male = aggregate_over_age(tb_agg, start_year=2010, end_year=END_YEAR, gender="male")
    ...
    # remove years before 2010 from tb_years (to keep it from showing up in grapher)
    tb_years = tb_years[tb_years["year"] >= 2010]
    ...
    ds_garden = paths.create_dataset(
        tables=[tb, tb_years], check_variables_metadata=True, default_metadata=ds_meadow.metadata
    )
