...
def run(dest_dir: str) -> None:
    ...
    tb = ds_meadow["historical_statistics_for_mineral_and_material_commodities"].reset_index()
    ...
    tb_unit_value = (
        ...
        .assign(**{"country": "World"})
        .rename(columns={"unit_value_dollar_t": "unit_value"}, errors="raise")
    )
    # Remove spurious footnotes like "W" (and "nan", caused by saving the column as string).
    tb_unit_value["unit_value"] = tb_unit_value["unit_value"].astype(str).replace("W", None).replace("nan", None)
