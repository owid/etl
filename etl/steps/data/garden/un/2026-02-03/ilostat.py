...
COLUMN_CATEGORIES = {
    ...
    "classif1": {
        ...
        "Age (Child labour bands): '5-11": "Age (child labor): 5-11",
        "Age (Child labour bands): '5-17": "Age (child labor): 5-17",
        "Age (Child labour bands): 12-14": "Age (child labor): 12-14",
        "Age (Child labour bands): 15-17": "Age (child labor): 15-17",
...
def add_modeled_category_to_obs_status(tb: Table) -> Table:
    ...
    for col in OBS_STATUS_COLUMNS:
        ...
        if obs_status_col in tb.columns:
            ...
            tb.loc[mask_modeled, obs_status_col] = "Estimated by ILO models (modeled)"

        # Replace "Real value" with "Survey data (observed)"
        mask_survey = tb[obs_status_col] == "Real value"
        tb.loc[mask_survey, obs_status_col] = "Survey data (national)"
