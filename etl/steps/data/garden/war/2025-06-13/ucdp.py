...
def run_pipeline(
    ...
    assert len(tb_summary) / tb["conflict_new_id"].nunique() <= tolerance_unk_ctype, (
...
def _get_location_of_conflict_in_ucdp_ged(tb: Table, tb_maps: Table, num_missing_location: int) -> Table:
    ...
    assert (num_err := tb[COLUMN_COUNTRY_NAME].isna().sum()) <= num_err_expected_max, (
...
def _sanity_check_prio_conflict_types(tb_prio: Table):
    ...
    assert not (tb_prio.groupby(["conflict_id", "year"])["type_of_conflict"].nunique() > 1).any(), (
        "Seems like the conflict has multiple types for a single year! Is it categorised differently depending on the region?"
    )
