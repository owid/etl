...
def patch_unknown_conflict_type_ced(tb):
    ...
    assert (tb.loc[mask, "year"] > LAST_YEAR_STABLE).all(), (
...
def _get_location_of_conflict_in_ucdp_ged(tb: Table, tb_maps: Table) -> Table:
    ...
    assert (diff := gdf.shape[0] - gdf_match.shape[0]) <= 2280, (
        f"Unexpected number of events without exact coordinate match! {diff}"
    )
