...
# Last year of data
LAST_YEAR = 2023
LAST_YEAR_PREVIEW = 2024  # datetime.now().year
...
def run() -> None:
    ...
    pipeline.run(
        last_year_actual=LAST_YEAR_PREVIEW,
        last_year=LAST_YEAR,
    )
...
class UCDPPipeline:
    ...
    def run(self, last_year_actual: int, last_year: int):
        # Sanity checks (2)
        assert (
            self.tb_conflict["year"].max() == last_year
        ), f"Unexpected max year in `ucdp_battle_related_conflict` ({self.tb_conflict['year'].max()})!"
        assert (
            self.tb_prio["year"].max() == last_year
        ), f"Unexpected max year in `ucdp_prio_armed_conflict` ({self.tb_prio['year'].max()})!"

        # Keep only active conflicts
        paths.log.info("keep active conflicts")
        self.tb_ged = self.tb_ged.loc[self.tb_ged["active_year"] == 1]

        # Change region named "Asia" to "Asia and Oceania" (in GED)
        self.tb_ged["region"] = self.tb_ged["region"].replace({"Asia": "Asia and Oceania"})

        # Create `conflict_type` column
        paths.log.info("add field `conflict_type`")
        tb = self.add_conflict_type(self.tb_ged, self.tb_conflict)
        ...
        assert (
            len(tb_summary) / tb["conflict_new_id"].nunique() < 0.01
        ...
        tb_participants = self.estimate_metrics_participants(tb)
        tb_locations = self.estimate_metrics_locations(tb, last_year)
        ...
        self._sanity_check_conflict_types(tb, until_year=last_year)
        self._sanity_check_prio_conflict_types()
        ...
        self.tb_prio = prepare_prio_data(self.tb_prio)

        # Fill NaNs
        paths.log.info("replace missing data with zeros (where applicable)")
        self.tb_prio = expand_time_column(
            self.tb_prio,
        ...
        tb = self.combine_with_table_prio(tb, last_year_actual)
    ...
    def add_conflict_type(self, tb_ged: Table, tb_conflict: Table) -> Table:
        ...
        tb_ged = self.patch_unknown_conflict_type_ced(tb_ged)
    ...
    def _sanity_check_conflict_types(self, tb: Table, until_year: Optional[int] = None) -> Table:
        """Check conflict type.
    ...
    def _sanity_check_prio_conflict_types(self) -> Table:
        ...
        # Get conflicts with more than one conflict type assigned to them over their lifetime
        conflict_type_transitions = self.tb_prio.groupby("conflict_id")["type_of_conflict"].apply(set)
        transitions = conflict_type_transitions[conflict_type_transitions.apply(len) > 1].drop_duplicates()
        # Extract unique combinations of conflict_types for a conflict
        ...
        # Check if different regions categorise the conflict differently in the same year
        assert not (
            self.tb_prio.groupby(["conflict_id", "year"])["type_of_conflict"].nunique() > 1
    ...
    def estimate_metrics_participants(self, tb: Table) -> Table:
        ...
        # Add country name
        tb_country["country"] = tb_country.apply(lambda x: self.tb_codes.loc[(x["id"], x["year"])], axis=1)
        ...
        tb_codes_ = self.tb_codes.reset_index().merge(tb_alltypes, how="cross")
        ...
        # Add PRIO #
        ############
        tb_country_prio = self.estimate_metrics_participants_prio()

        tb_country = pr.concat(
            [tb_country, tb_country_prio], ignore_index=True, short_name=f"{paths.short_name}_country"
        )

        return tb_country

    def estimate_metrics_participants_prio(self) -> Table:
        """Add participant information at country-level.
        ...
        tb_country = pr.concat(
            [self.tb_prio[["year", "type_of_conflict", code]].rename(columns={code: "id"}).copy() for code in codes]
        )
        ...
        # Add flag
        tb_country["participated_in_conflict"] = 1
        tb_country["participated_in_conflict"].m.origins = self.tb_prio["gwno_a"].m.origins
        ...
        tb_codes = self.tb_codes.reset_index().merge(tb_alltypes, how="cross")
        ...
        tb_country = tb_country[
            (tb_country["year"] >= self.tb_prio["year"].min()) & (tb_country["year"] <= self.tb_prio["year"].max())
        ]
    ...
    def estimate_metrics_locations(self, tb: Table, last_year: int) -> Table:
        """Add participant information at country-level.
        ...
        """
        tb_codes_ = self.tb_codes.reset_index().drop(columns=["id"]).copy()
        ...
        tb_locations = self._get_location_of_conflict_in_ucdp_ged(tb).copy()
        ...
        tb_locations.loc[tb_locations["country_name_location"] == "Siachen Glacier", "country_name_location"] = (
            "Pakistan"
        )
        ...
        assert (
            tb_locations["best"]
            - tb_locations[["deaths_a", "deaths_b", "deaths_civilians", "deaths_unknown"]].sum(axis=1)
            == 0
        ).all(), "Sum of deaths from combatants, civilians and unknown should equal best estimate!"
        ...
        assert (
            tb_locations_country["is_location_of_conflict"].notna().all()
        ), "Missing values in `is_location_of_conflict`!"
        ...
        for col in cols:
            tb_locations_country[col].origins += self.tb_maps["name"].m.origins
        ...
        tb_locations_regions_world = _get_number_of_locations_with_conflict_regions(
            tb_locations, ["year", "conflict_type"]
        )
    ...
    def _get_location_of_conflict_in_ucdp_ged(self, tb: Table) -> Table:
        ...
        gdf_maps = gpd.GeoDataFrame(self.tb_maps)
    ...
    def combine_with_table_prio(self, tb: Table, last_year: int) -> Table:
        ...
        assert self.tb_prio["year"].min() == 1946, "Unexpected start year!"
        assert self.tb_prio["year"].max() == 1989, "Unexpected start year!"
        ...
        self.tb_prio.loc[self.tb_prio["year"] == 1989, "number_ongoing_conflicts"] = np.nan

        # Merge Geo with UCDP/PRIO
        tb = self.tb_prio.merge(tb, on=["year", "region", "conflict_type"], suffixes=("_prio", "_main"), how="outer")
...
class UCDPPipelineCED(UCDPPipeline):
    def patch_unknown_conflict_type_ced(self, tb):
