...
def run(dest_dir: str) -> None:
    ...
    ds_meadow = paths.load_dataset("total_snowfall")
    tb = ds_meadow["total_snowfall"].reset_index()
    ...
    tb["days_in_month"] = tb["days_in_month"].copy_metadata(tb["total_snowfall"])
    ...
    tb["total_snowfall"] = tb["total_snowfall"] * 1000 * tb["days_in_month"]
    ...
    tb_baseline = tb_baseline.groupby(["country", "month"], as_index=False)["total_snowfall"].mean()
    tb_baseline = tb_baseline.rename(columns={"total_snowfall": "mean_total_snowfall"})
    ...
    tb["snowfall_anomaly"] = tb["total_snowfall"] - tb["mean_total_snowfall"]

    tb = tb.drop(columns=["month", "year", "mean_total_snowfall"])
