...
def run(dest_dir: str) -> None:
    ...
    #
    # Load meadow dataset.
    ds_meadow = paths.load_dataset("plastic_aquatic_leakage_projections")

    # Read table from meadow dataset.
    tb = ds_meadow["plastic_aquatic_leakage_projections"].reset_index()
    #
    # Process data.
    #
    ...
    tb = tb.groupby(["year", "water_source", "country"])["value"].sum().reset_index()
    # Add the metadata back to the table
    tb.metadata = metadata
    # Calculate the global totals
    total_df = tb.groupby(["year", "water_source"])["value"].sum().reset_index()

    total_df["country"] = "World"
    combined_df = pr.merge(total_df, tb, on=["country", "year", "water_source", "value"], how="outer").copy_metadata(
        from_table=tb
    )
    total_df = total_df.rename(columns={"value": "global_value"})

    # Merge the global totals back to the original DataFrame
    df_with_share = pr.merge(combined_df, total_df, on=["year", "water_source"], how="left")
    ...
    tb = df_with_share.underscore().set_index(["country", "year", "water_source"], verify_integrity=True).sort_index()
