...
CATEGORIES = {
    "dac1": {
        "aid_type": {
            "ODA flows basis, as percent of GNI": {"new_name": "oda_share_gni"},
            "ODA grant equivalent as percent of GNI": {
                "new_name": "oda_share_gni",
            },
...
def run(dest_dir: str) -> None:
    ...
    tb_dac1 = remove_jumps_in_the_data(tb=tb_dac1)
...
def create_indicators_as_share_of_gni(tb: Table, indicator_list: List[str]) -> Table:
    ...
    for indicator in indicator_list:
        tb[f"{indicator}_share_gni_2"] = tb[indicator] / tb["gni_net_disbursements"] * 100
...
def add_donor_data_from_recipient_dataset(tb_donor: Table, tb_recipient: Table) -> Table:
    ...
    # Rename oda
    tb = tb.rename(columns={"oda": "oda_inverted"})
...
def remove_jumps_in_the_data(tb: Table) -> Table:
    """
    ...
    # For i_oda_net_disbursements_share_gni_2
    tb.loc[
        (tb["country"] == "Non-DAC countries (OECD)") & (tb["year"].isin([1990, 1991])),
