...
def run(dest_dir: str) -> None:
    log.info("orae_1985: starting")

    #
    # Load inputs.
    #
    log.info("orae_1985: loading inputs")
    # Load meadow dataset.
    ds_meadow: Dataset = paths.load_dependency("orae_1985")
    # Read table from meadow dataset.
    tb_meadow = ds_meadow["orae_1985"]
    ...
    # Process data.
    #
    log.info("orae_1985: processing dataframe")
    df = clean_data(df)

    # Create a new table with the processed data.
    log.info("orae_1985: generating tables")
    tables = make_tables(df, paths.short_name)

    #
    # Save outputs.
    #
    log.info("orae_1985: adding tables to dataset")
