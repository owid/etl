...
def run(dest_dir: str) -> None:
    log.info("sutton_1971: starting")

    #
    # Load inputs.
    #
    log.info("sutton_1971: loading inputs")
    # Load meadow dataset.
    ds_meadow: Dataset = paths.load_dependency("sutton_1971")
    # Read table from meadow dataset.
    tb_meadow = ds_meadow["sutton_1971"]
    ...
    # Process data.
    #
    log.info("sutton_1971: processing dataframe")
    df = clean_data(df)

    # Create a new table with the processed data.
    log.info("sutton_1971: generating tables")
    tables = make_tables(df, paths.short_name)

    #
    # Save outputs.
    #
    log.info("sutton_1971: adding tables to dataset")
