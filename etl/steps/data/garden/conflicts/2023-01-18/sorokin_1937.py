...
def run(dest_dir: str) -> None:
    log.info("sorokin_1937: starting")

    #
    # Load inputs.
    #
    log.info("sorokin_1937: loading inputs")
    # Load meadow dataset.
    ds_meadow: Dataset = paths.load_dependency("sorokin_1937")
    # Read table from meadow dataset.
    tb_meadow = ds_meadow["sorokin_1937"]
    ...
    # Process data.
    #
    log.info("sorokin_1937: processing dataframe")
    df = clean_data(df)

    # Create a new table with the processed data.
    log.info("sorokin_1937: generating tables")
    tables = make_tables(df, paths.short_name)

    #
    # Save outputs.
    #
    log.info("sorokin_1937: adding tables to dataset")
