"""Greenhouse gas emissions by human activity, splitting electricity into the activities that consume it.

This step loads ``emissions_by_human_activity`` (where electricity is its own activity) and redistributes
the electricity emissions into the activities that consume the electricity, using UNdata's Energy
Statistics Database (final electricity consumption by sector). It mirrors the two input tables:

1. ``emissions_by_human_activity_splitting_electricity`` (electricity split; "Other" folded in).
2. ``emissions_by_human_activity_splitting_electricity_including_other`` (electricity split; "Other" shown).

For each activity we report direct emissions (on-site, from the input step), indirect emissions (the
share of electricity-generation emissions attributable to that activity's electricity use), and their
total. Only greenhouse-gas emissions are produced here (the electricity split uses GHG).
"""
...
def run() -> None:
    ...
    # Load inputs.
    #
    # Load the base step (electricity shown as its own activity) and read both tables.
    ds_base = paths.load_dataset("emissions_by_human_activity")
    tb_base = ds_base.read("emissions_by_human_activity")
    tb_base_other = ds_base.read("emissions_by_human_activity_including_other")
    ...
    tb_other = split_electricity(tb_base_other, shares_by_activity(tb_shares, ACTIVITY_FROM_INTERMEDIATE))

    # Sanity checks.
    sanity_check_outputs(tb, activities=[a for a in tb_base["sector"].unique() if a != "Electricity"])
    sanity_check_outputs(tb_other, activities=[a for a in tb_base_other["sector"].unique() if a != "Electricity"])

    # Improve table format.
    tb = tb.format(keys=["country", "year", "sector"], short_name="emissions_by_human_activity_splitting_electricity")
    tb_other = tb_other.format(
        keys=["country", "year", "sector"],
        short_name="emissions_by_human_activity_splitting_electricity_including_other",
    )
