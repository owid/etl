...
def check_gini_before_after_tax(tb_inequality: Table) -> None:
    """
    Check that Gini after tax (dhi) is lower than Gini before tax (mi).
    Only compares country-year pairs where both welfare types have data.
    """
    tb = tb_inequality.reset_index()

    for equivalence_scale in tb["equivalence_scale"].unique():
        tb_before = tb[(tb["welfare_type"] == "mi") & (tb["equivalence_scale"] == equivalence_scale)][
            ["country", "year", "gini"]
        ].rename(columns={"gini": "gini_before"})

        tb_after = tb[(tb["welfare_type"] == "dhi") & (tb["equivalence_scale"] == equivalence_scale)][
            ["country", "year", "gini"]
        ...
        if mask.any():
            ...
            paths.log.warning(
                f"""{len(tb_error)} observations ({equivalence_scale}) where Gini after tax >= Gini before tax:
