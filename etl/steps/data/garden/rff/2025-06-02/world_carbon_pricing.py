...
def run() -> None:
    ...
    tb_national = ds_meadow.read("world_carbon_pricing_national_level", safe_types=False)
    tb_subnational = ds_meadow.read("world_carbon_pricing_subnational_level", safe_types=False)
    ...
    tb = tb.format(
        keys=["country", "year", "ipcc_code", "product"], sort_columns=True, short_name="world_carbon_pricing"
    )
    tb_any_sector = tb_any_sector.format(
        keys=["country", "year"], sort_columns=True, short_name="world_carbon_pricing_any_sector"
    )
    ...
    ds_garden = paths.create_dataset(tables=[tb, tb_any_sector], default_metadata=ds_meadow.metadata)
    ds_garden.save()
