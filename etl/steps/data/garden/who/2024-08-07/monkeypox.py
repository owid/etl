...
def clean_values(tb: Table) -> Table:
    ...
    tb["total_cases"] = tb[["location", "total_cases"]].groupby("location", observed=True).cummin()
    tb["total_deaths"] = tb[["location", "total_deaths"]].groupby("location", observed=True).cummin()
    return tb.sort_values(["location", "date"])
...
def explode_dates(tb: Table) -> Table:
    tb_range = pd.concat(
        [
            ...
            for location in tb.location.unique()
        ]
    )
    tb = pr.merge(tb, Table(tb_range), on=["location", "date"], validate="one_to_one", how="right")
...
def add_world(tb: Table) -> Table:
    tb[["total_cases", "total_deaths"]] = (
        tb[["location", "total_cases", "total_deaths"]].groupby("location", observed=True).ffill().fillna(0)
    )
    world = (
...
def add_regions(tb: Table) -> Table:
    ...
    for region in [
        ...
        tb.loc[tb.location.isin(list_countries_in_region(region=region)), "region"] = region
...
def add_population_and_countries(tb: Table) -> Table:
    ...
    missing_locs = set(tb.location) - set(pop.location)
    if len(missing_locs) > 0:
        raise Exception(f"Missing location(s) in population file: {missing_locs}")
    assert tb.m.short_name
    tb = pr.merge(tb, Table(pop), how="left", validate="many_to_one", on="location")
    return tb
