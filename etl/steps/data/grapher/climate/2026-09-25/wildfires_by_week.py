...
def run() -> None:
    ...
    # Process data.
    #
    tb["date"] = pr.to_datetime(tb["date"])
    ...
    tb_pivot_events = process_data(tb, "events", "Number of wildfires in ")
    ...
    ds_grapher = paths.create_dataset(tables=[tb_pivot_all], default_metadata=ds_garden.metadata)
