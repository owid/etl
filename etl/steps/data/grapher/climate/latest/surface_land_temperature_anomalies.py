...
def run(dest_dir: str) -> None:
    ...
    for column in tb_merged.columns:
        ...
        tb_merged[column].metadata.origins = tb["temperature_anomaly"].metadata.origins
        tb_merged[column].metadata.unit = tb["temperature_anomaly"].metadata.unit
        tb_merged[column].metadata.short_unit = tb["temperature_anomaly"].metadata.short_unit
        tb_merged[column].metadata.description_short = tb["temperature_anomaly"].metadata.description_short
        tb_merged[column].metadata.description_from_producer = tb[
            "temperature_anomaly"
        ].metadata.description_from_producer
        tb_merged[column].metadata.description_processing = tb["temperature_anomaly"].metadata.description_processing
    ...
    ds_grapher.metadata.title = "Monthly land surface temperature anomalies since 1950 by country"
    ds_grapher.save()
