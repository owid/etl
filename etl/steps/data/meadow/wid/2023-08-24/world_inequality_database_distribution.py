...
def harmonize_countries(df: pd.DataFrame, iso2_missing: dict) -> pd.DataFrame:
    ...
    for x, y in iso2_missing.items():
        # NOTE: See if I can do this without numpy
...
def run(dest_dir: str) -> None:
    ...
    # List of datasets with their snapshot
    # Retrieve snapshot.
    snap = cast(Snapshot, paths.load_dependency("world_inequality_database_distribution.csv"))
    ...
    snap: Snapshot = paths.load_dependency("world_inequality_database_distribution_with_extrapolations.csv")
    # Load data from snapshot.
    # `keep_default_na` and `na_values` are included because there is a country labeled NA, Namibia, which becomes null without the parameters
    ...
    df = pd.merge(
        df,
        df_extrapolations[["country", "year", "welfare", "percentile", "thr", "avg", "share"]],
        on=["country", "year", "welfare", "percentile"],
        how="outer",
