...
def run(dest_dir: str) -> None:
    ...
    for tb_name, tb_ids in snapshots_dict.items():
        ...
        # Set index and sort
        tb = tb.set_index(tb_ids, verify_integrity=True).sort_index()
    ...
    ds_meadow = create_dataset(
        dest_dir, tables=tables, check_variables_metadata=True, default_metadata=snap_main.metadata
    )
...
def harmonize_countries(tb: Table, tb_regions: Table, iso2_missing: dict, iso_2_missing_mer: dict) -> Table:
    ...
    if missing_count > 0:
        log.fatal(
            f"There are still {missing_count} unnamed WID countries/regions! Take a look at this list:\n {missing_list}"
        )
