...
def format_income_tables(tb: List[Table], aggregation: List[str]) -> List[Table]:
    """Format the income table."""

    tables = []
    for t in tb:
        # Fill columns with data for spells
        t.loc[0] = t.loc[0].fillna(method="ffill")

        # Flatten column names
        t.columns = ["_".join(map(str, x)) if isinstance(x, tuple) else str(x) for x in t.columns.values]

        # Rename columns as the current name + the first row
        t.columns = t.columns + "_" + t.loc[0].astype(str)
        ...
        # Rename first column to "indicator"
        t = t.rename(columns={"Country_Year_nan": "indicator"})

        # Create column aggregation, by selecting indicator where values are [NATIONAL, URBAN, RURAL]. Fill empty values with the previous value
        t["aggregation_level"] = t.loc[t["indicator"].isin(aggregation), "indicator"]
        t["aggregation_level"] = t["aggregation_level"].fillna(method="ffill")

        # Drop rows with indicator = null
        t = t.dropna(subset=["indicator"]).reset_index(drop=True)

        # Drop rows with indicator = [NATIONAL, URBAN, RURAL]
        t = t[~t["indicator"].isin(aggregation)].reset_index(drop=True)

        # If values in indicator are between 1 and 10, add "decile" to the indicator
        t.loc[t["indicator"].astype(str).str.isnumeric(), "indicator"] = "decile" + t["indicator"].astype(str) + "_thr"

        # Remove last row if it is SOURCE_TEXT
        assert t.iloc[-1, 0] == SOURCE_TEXT, f"Last row is not {SOURCE_TEXT}. Check the file."

        t = t.drop(t.index[-1]).reset_index(drop=True)

        # Make table wider, using aggregation_level as a column
        t = t.pivot(index="indicator", columns="aggregation_level").reset_index()

        # Flatten column names
        t.columns = ["_".join(map(str, x)) if isinstance(x, tuple) else str(x) for x in t.columns.values]

        # Transpose the table
        t_transposed = t.copy().T.reset_index()

        # Copy metadata to each column
        for col in t_transposed.columns:
            t_transposed[col] = t_transposed[col].copy_metadata(t["indicator_"])

        # Nane columns with the first row
        t_transposed.columns = t_transposed.iloc[0].astype(str).tolist()

        # Remove the first row
        t_transposed = t_transposed.drop(0).reset_index(drop=True)

        # Separate indicator_ column by _
        t_transposed[["country", "year", "survey", "aggregation"]] = t_transposed["indicator_"].str.split(
            "_", expand=True
        )

        # Drop indicator_ column
        t_transposed = t_transposed.drop(columns="indicator_")

        # Ensure all columns are snake-case, set an appropriate index, and sort conveniently.
        t_transposed = (
            t_transposed.underscore()
            .set_index(["country", "year", "survey", "aggregation"], verify_integrity=True)
            .sort_index()
        )

        tables.append(t_transposed)
