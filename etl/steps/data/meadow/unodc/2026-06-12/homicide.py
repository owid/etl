...
def clean_data_regional(tb_region: Table) -> Table:
    ...
    # The new data separates regions and subregions into two columns.
    # For regional totals (subregion == "All"), use "region" as the country name.
    # For subregional breakdowns (subregion != "All"), use "subregion" as the country name.
    tb_reg_totals = tb_region[tb_region["subregion"] == "All"].copy()
    tb_reg_totals = tb_reg_totals.rename(columns={"region": "country"}, errors="raise")

    tb_reg_sub = tb_region[tb_region["subregion"] != "All"].copy()
    tb_reg_sub = tb_reg_sub.rename(columns={"subregion": "country"}, errors="raise")
    tb_reg_sub = tb_reg_sub.drop(columns=["region"])

    tb_region = pr.concat([tb_reg_totals, tb_reg_sub], ignore_index=True)
    tb_region = tb_region.drop(columns=["subregion"], errors="ignore")
