...
def run() -> None:
    ...
    # Use the latest year in the annual data as the label for cumulative cause/operation totals
    latest_year = int(nsp_quant["year"].max())

    df_above_7000_cause_totals_pv = extract_cause_totals(df_above_7000, latest_year, "Large (>700t)")
    df_below_7000_cause_totals_pv = extract_cause_totals(df_7_7000, latest_year, "Medium (7-700t)")
    ...
    ops_df = extract_operations(df_above_7000, df_7_7000, latest_year)
...
def extract_spill_quantity(text):
    ...
    # Remove the row where year is 2025 and the value is 7 - this is just a page number
    df_oil_spills = df_oil_spills[(df_oil_spills["year"] != 2025) | (df_oil_spills["oil_spilled"] != 7)]
...
def extract_spill_number(text):
    ...
    pattern = r"(?<!\d)(19[7-9]\d|20[0-1]\d|2020|2021|2022|2023|2024|2025)(?:\s+(\d+)\s+(\d+))?"
...
def prepare_ops_dataframe(df, spill_type, year):
    """
    ...
    year : int
        The latest year in the annual data, used as the label for cumulative totals.
...
def extract_operations(df_above_7000, df_7_7000, year):
    """
    ...
    ----
    """
    operations_ab_7000 = prepare_ops_dataframe(df_above_7000, "Large (>700t)", year)
    operations_bel_7000 = prepare_ops_dataframe(df_7_7000, "Medium (7-700t)", year)
