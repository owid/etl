...
def run(dest_dir: str) -> None:
    ...
    #
    # Retrieve snapshot.
    snap = cast(Snapshot, paths.load_dependency("monmoth_poll.pdf"))

    # Load data from snapshot.
    start_page = 3  # tables of interest start on page 3
    end_page = 6  # and end on page 6
    texts, dataframes = extract_text_and_tables_from_pdf(snap.path, start_page, end_page)
    ...
    # pivot to have
    pivot_df = combined_df.pivot_table(index=["answer", "year"], columns="question", values="percentage").reset_index()
...
def extract_text_and_tables_from_pdf(pdf_path, start_page=None, end_page=None):
    """
    Extracts survey questions and tables from a PDF file within the specified page range.
...
def preprocess_data(dataframes, texts):
    """
    Preprocesses a list of dataframes by replacing 'None' values with NaNs, extracting column names from the row with '2023',
    dropping rows with NaN values, dropping rows with '(n)' in the index, adding the 'question' and 'country' columns,
    converting the 'percentage' column to float, and melting the dataframe.
