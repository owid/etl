...
# Path glob within the ECP compressed folder to the economy data file (fixed-weights variant).
# v2026.1 splits the economy file into ecp_fw (fixed weights), ecp_vw (variable weights), and
# ecp_intro (jurisdictional only). The fixed-weights file preserves the column shape of the old
# single `ecp_CO2.csv`. The filename now carries a build-date suffix that changes per release, so
# we glob the directory rather than hard-coding the date.
DATA_PATH_ECONOMY_GLOB = "ECP-main/_output/_dataset/v2026.1/ecp/ipcc/ecp_economy/ecp_fw/ecp_fixed_CO2_*.csv"
