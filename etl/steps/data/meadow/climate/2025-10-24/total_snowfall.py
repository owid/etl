...
def _load_data_array(snap: Snapshot) -> xr.DataArray:
    ...
    da = da["sf"]
    # Set the coordinate reference system for the snowfall data to EPSG 4326.
...
def run(dest_dir: str) -> None:
    ...
    #
    # Retrieve snapshot.
    snap = paths.load_snapshot("total_snowfall.zip")
    ...
    # Initialize an empty dictionary to store the country-wise average snowfall.
    temp_country = {}

    # Add Global mean snowfall
    ...
    # Initialize a list to keep track of small countries where snowfall data extraction fails.
    small_countries = []
    ...
    for i in tqdm(range(shapefile.shape[0])):
        ...
        try:
            ...
            # Apply the weights to the clipped snowfall data.
            clim_month_weighted = clip.weighted(weights)

            # Calculate the weighted mean snowfall for the country.
            country_weighted_mean = clim_month_weighted.mean(dim=["longitude", "latitude"]).values

            # Store the calculated mean snowfall in the dictionary with the country's name as the key.
    ...
    # Log information about countries for which snowfall data could not be extracted.
    log.info(
        f"It wasn't possible to extract snowfall data for {len(small_countries)} small countries as they are too small for the resolution of the Copernicus data."
    )
    # Define the start and end dates from valid_time coordinate
    start_time = pd.to_datetime(da["valid_time"].min().values).date()
    end_time = pd.to_datetime(da["valid_time"].max().values).date()
    ...
    melted_df = df_temp.melt(id_vars=["time"], var_name="country", value_name="total_snowfall")
    ...
    tb["total_snowfall"].metadata.origins = [snap.metadata.origin]
    #
    # Save outputs.
    #
