...
def run() -> None:
    ...
    #
    # Retrieve snapshot.
    snap_1 = paths.load_snapshot("nasa_ozone_hole_extremes.txt")
    snap_2 = paths.load_snapshot("nasa_ozone_hole_annual.txt")
    ...
    ds_meadow = paths.create_dataset(
        tables=[tb],
        default_metadata=snap_1.metadata,
    )
...
def build_df_extremes(snap: Snapshot) -> pd.DataFrame:
    """Read raw data from the year-to-date file (annual max hole area and min concentration)."""
    paths.log.info("ozone_hole_area: loading extremes (ytd) data")
    # Read data
...
def build_df_annual(snap: Snapshot) -> pd.DataFrame:
    """Read raw data from the annual file (annual mean hole area and concentration)."""
    paths.log.info("ozone_hole_area: loading annual-mean data")
    # Read data
    text = _read_txt_without_comments(snap.path)
    # Sanity checks
    ...
    text = [re.split(r"\s+", t.strip()) for t in text]
    # Build dataframe
    df = pd.DataFrame.from_records(text, columns=["year", "mean_hole_area", "min_mean_hole_concentration"])
    return df
...
def build_df(snap_1: Snapshot, snap_2: Snapshot) -> pd.DataFrame:
    ...
    # Load two sources
    df1 = build_df_extremes(snap_1)
    df2 = build_df_annual(snap_2)
    # Merge
    ...
    df = df.astype(
        {
            ...
            "min_mean_hole_concentration": "Float32",
        }
    )
