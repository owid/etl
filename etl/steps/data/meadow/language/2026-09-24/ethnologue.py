...
def run() -> None:
    ...
    for file_name, index, short_name in zip(file_names, indexes, short_names):
        with zipfile.ZipFile(snap.path, "r") as zip_ref:
            # The files sit in a release-dated folder (e.g. Language_Code_Data_20260217/), so locate it rather than hardcode it.
            matches = [n for n in zip_ref.namelist() if n.endswith(f"/{file_name}") and "__MACOSX" not in n]
            assert len(matches) == 1, f"Expected exactly one {file_name} in the zip, found: {matches}"
            with zip_ref.open(matches[0]) as file:
