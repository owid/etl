...
category_structure = {
    ...
    # Specific for faostat_sdgb.
    "areagroups": {
        "index": ["Area Group Code", "Area Code"],
        "short_name": "area_group",
    },
}
...
def check_that_category_structure_is_well_defined(additional_metadata: Dict[str, Any]) -> None:
    ...
    """
    for dataset in list(additional_metadata):
        for category in category_structure:
            category_indexes = category_structure[category]["index"]
            if category in additional_metadata[dataset]:
                category_metadata = additional_metadata[dataset][category]["data"]
...
def run(dest_dir: str) -> None:
    ...
    #
    # Run sanity checks.
    check_that_category_structure_is_well_defined(additional_metadata=additional_metadata)
