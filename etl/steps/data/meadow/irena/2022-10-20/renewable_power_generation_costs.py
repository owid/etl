...
def prepare_pv_data(data_file: str) -> pd.DataFrame:
    ...
    pv_prices = pd.read_excel(data_file, sheet_name="Fig 3.2", skiprows=4, skipfooter=18,
                              usecols=lambda column: 'Unnamed' not in column)

    # Transpose dataframe so that each row corresponds to a month.
    pv_prices = pv_prices.rename(columns={"2021 USD/W": "technology"}).\
        melt(id_vars="technology", var_name="month", value_name="cost")
    ...
    pv_prices = pv_prices.groupby(["technology", "year"]).agg({"cost": "mean", "year": "count"}).rename(columns={"year": "n_months"}).reset_index()
