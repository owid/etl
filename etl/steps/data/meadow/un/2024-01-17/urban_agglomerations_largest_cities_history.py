...
def run(dest_dir: str) -> None:
    ...
    #
    # Retrieve snapshot.
    snap = paths.load_snapshot("urban_agglomerations_largest_cities_history.xls")
    ...
    tb = tb.rename(columns={"Country or Area": "country", "Rank\nOrder": "rank"})
    # Ensure all columns are snake-case, set an appropriate index, and sort conveniently.

    tb = (
        tb.underscore()
        .set_index(["country", "year", "urban_agglomeration", "rank"], verify_integrity=True)
        .sort_index()
    )
    #
    # Save outputs.
