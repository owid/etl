...
def run(dest_dir: str) -> None:
    ...
    # Combine famines for the African Red Sea Region and Hungerplan as the mortality estimate exists for just the total rather than each entry (keep the originals there as well for making the famine count plots)
...
def combine_entries(tb):
    ...
    rows_to_combine = tb[tb["Conventional title"].apply(lambda x: any(sub in x for sub in famines))]

    # Combine dates
    combined_dates = ",".join(rows_to_combine["Date"].unique())

    # Calculate the sum of the 'WPF authoritative mortality estimate' column (should actually just be one value and nans)
    ...
    # Additional logic to combine "Hungerplan" entries as the mortality estimate only exists for the total
    places = ["USSR (Russia)", "USSR (Ukraine)", "USSR (Russia and Western Soviet States)"]
    famine = "Hungerplan (German)"

    hungerplan_rows = tb[
        tb["Conventional title"].apply(lambda x: any(sub in x for sub in famine)) & tb["Place"].isin(places)
    ]
    if not hungerplan_rows.empty:
        ...
        tb = tb._append(new_hungerplan_entry, ignore_index=True)
