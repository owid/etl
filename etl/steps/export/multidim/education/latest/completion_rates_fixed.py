...
def run() -> None:
    ...
    # Match full column names that contain all required parts
    female = [col for col in tb.columns if "completion_rate" in col and "female" in col and "modelled_data" in col]
    male = [col for col in tb.columns if "completion_rate" in col and "male" in col and "modelled_data" in col]

    completion_cols = both_sexes + female + male
    # Select only relevant columns
    tb = tb[["country", "year"] + completion_cols]

    # Adjust dimensions
    tb = adjust_dimensions_completion(tb)
...
def adjust_dimensions_completion(tb):
    """
    Add dimensions to completion table columns.

    """

    # Helper maps for pattern matching (excluding pre-primary and tertiary)
    level_keywords = {
        "primary": "primary",
...
def generate_title_by_gender_and_level(sex, level):
    ...
    return f"Completion rate for {gender_term} in {level_term}"
    ...
    if view.dimensions.get("level") == "level_side_by_side":
        if view.indicators.y is not None:
            for indicator in view.indicators.y:
                ...
                if "completion" in indicator.catalogPath and "primary" in indicator.catalogPath:
                    display_name = "Primary"
                elif "completion" in indicator.catalogPath and "lower_secondary" in indicator.catalogPath:
                    display_name = "Lower secondary"
                elif "completion" in indicator.catalogPath and "upper_secondary" in indicator.catalogPath:
