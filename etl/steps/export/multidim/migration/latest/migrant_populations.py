...
# Population (x, share views only) and color (region) indicators needed by the Marimekko tab.
...
def run() -> None:
    ...
    # Add Marimekko as an additional chart type, colored by region. Share views are also
    # sized by population, since a share alone says nothing about the number of people behind it.
    for view in c.views:
        view.config = view.config or {}
        view.config["chartTypes"] = ["LineChart", "DiscreteBar", "Marimekko"]
        if view.matches(unit="share"):
