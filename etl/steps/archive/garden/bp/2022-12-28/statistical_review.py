...
We use BP's "Other *" regions to create continent aggregates of each variable, but remove them afterwards, given that
these regions mean a different set of countries for different variables.
...
def run(dest_dir: str) -> None:
    ...
    # Remove "Other *" regions. They have been properly taken into account to create continent aggregates, but they
    # represent different sets of countries for different variables.
    # Therefore we remove them to avoid issues when combining different variables from these regions.
    df = df[~df["country"].str.startswith("Other")].reset_index(drop=True)
